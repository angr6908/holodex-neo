...
export function formatDistance(
  ...
  $t: (k: string, a?: Record<string, string | number | Date>) => string,
  allowNeg = true,
  now = dayjs(),
) {
  if (!time) return "?";
  ...
  if (hrDiff < -23)
    return $t("time.diff_future_date", { arg0: t.format("l"), arg1: t.format("LT") });
  if (hrDiff > 23) return `${t.format("l")} (${t.format("LT")})`;
  if (t.isAfter(n))
    return $t("time.diff_future_date", {
      arg0: formatRelativeTime(t, lang, n),
      arg1: t.format(`${t.isTomorrow() ? "ddd " : ""}LT`),
    });
