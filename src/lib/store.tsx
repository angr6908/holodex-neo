...
function buildDefaultSettings(weblang = "en"): Settings {
  ...
  return normalizeSettings({
    lang: getUILang(weblang),
    clipLangs: [lang],
    liveTlLang: lang,
    useEnglishName: englishPrefs.has(lang),
  });
}
...
const KEYS = {
  SETTINGS: "holodex-v2-settings",
  APP: "holodex-v2-app",
  ORGS: "holodex-v2-orgs",
  HOME: "holodex-v2-home",
  FAVS: "holodex-v2-favorites",
  LIB: "holodex-v2-library",
...
const selectedOrgsFor = (s: State, org: Org) =>
  (s.selectedHomeOrgs?.length || 0) > 1
    ? s.selectedHomeOrgs
    : org?.name && org.name !== ALL_VTUBERS_ORG
      ? [org.name]
      : [];
...
function writeBootCookie(s: State) {
  ...
  const enc = encodeCookieJson({
    ...
    settings: {
      ...
      defaultOpen: s.settings.defaultOpen,
      homeViewMode: s.settings.homeViewMode,
      scrollMode: s.settings.scrollMode,
      hideUpcoming: s.settings.hideUpcoming,
      hideLive: s.settings.hideLive,
...
function loadPersisted(base: State): State {
  ...
  const settings = normalizeSettings(
    readJSON(KEYS.SETTINGS, buildDefaultSettings(navigator.language)),
  );
  const home = readJSON(KEYS.HOME, {
    live: [],
    lastLiveUpdate: 0,
    liveCacheKey: JSON.stringify([DEFAULT_ORG]),
  });
...
const youtubeIdOf = (v: any) =>
  v?.status === "live" &&
  v?.type !== "placeholder" &&
  !v?.link?.includes?.("twitch") &&
  /^[\w-]{11}$/.test(v?.id)
    ? (v.id as string)
    : null;

const mergeViewerCounts = (
  videos: any[],
...
export function AppStateProvider({
  ...
  useEffect(() => {
    ...
    if (enc && typeof document !== "undefined")
      document.cookie = `${HOME_STATE_COOKIE}=${enc}; Path=/; SameSite=Lax`;
  }, [hydrated, state.homeNav]);
  useEffect(() => {
    if (hydrated)
      writeJSON(KEYS.PLAYLIST, {
  ...
  const blockedChannelIDs = useMemo(
    () => new Set(state.settings.blockedChannels.map((x: any) => x.id).filter(Boolean)),
    [state.settings.blockedChannels],
  );
  const ignoredTopicsSet = useMemo(
