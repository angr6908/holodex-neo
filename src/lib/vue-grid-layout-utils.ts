...
export const getLayoutItem = (l: Layout, id: string | number) =>
  l.find((i) => String(i.i) === String(id));
...
function moveAway(layout: Layout, collidesWith: Item, toMove: Item, isUserAction: boolean): Layout {
  if (isUserAction) {
    ...
    if (!firstCollision(layout, fake))
      return moveElement(layout, toMove, undefined, fake.y, false, false);
  }
  return moveElement(layout, toMove, undefined, toMove.y + 1, false, false);
}

export const calcGridPosition = (
  x: number,
  y: number,
  w: number,
  h: number,
...
export const calcGridWH = (
  ...
  h: Math.max(
    Math.min(autoSize ? Math.ceil(height / rh) : Math.round(height / rh), maxRows - innerY),
    0,
  ),
});
