...
function normalizeAutocomplete(data: any) {
  ...
  const out: any[] = [];
  for (const v of data.vtuber || [])
    out.push({
      type: "channel",
      value: v.id,
      text: v.english_name || v.name,
      name: v.name,
...
export function jsonpItunes(
  query: string,
  { country = "JP", lang, limit = 10 }: { country?: string; lang?: string; limit?: number } = {},
...
export const api = {
  ...
  channelVideos: (
    id: string,
    { type = "videos", query = {} }: { type?: string; query?: Record<string, any> },
  ) => ax.get(`/channels/${id}/${type}?${qs(query)}`),
  login: (jwt: string | null, token: string, service: string) =>
    ax.post("/user/login", { token, service }, { headers: H(jwt) }),
  loginIsValid: (jwt: string | null) =>
    ax
      .get("/user/refresh", { headers: H(jwt) })
      .then((r) => (r.status === 200 && r.data.jwt && r.data.user ? r : false))
      .catch((e) => e.response || false),
  changeUsername: (jwt: string | null, name: string) =>
    ax.post("/user", { name }, { headers: H(jwt) }).catch(() => false),
  resetAPIKey: (jwt: string | null) =>
    ax
      .get("/user/createKey", { headers: H(jwt) })
      .catch(() => alert("something went wrong creating your key...")),
  favorites: (jwt: string | null) => ax.get("/users/favorites", { headers: H(jwt) }),
  favoritesVideos: (jwt: string | null, q: Record<string, any> = {}) =>
    ax.get(`/users/videos?${qs(q)}`, { headers: H(jwt) }),
  favoritesLive({ includePlaceholder = false } = {}, jwt: string | null) {
    return ax
      ...
      .then((r) =>
        r.data
          .filter(isLiveInWindow)
          .filter(
            (l: any) => l.start_actual || dayjs(l.start_scheduled).isBefore(dayjs().add(3, "w")),
          ),
      );
  },
  ...
  topicSet: (topicId: string, videoId: string, jwt?: string | null) =>
    ax.post("/topics/video", { videoId, topicId }, { headers: H(jwt) }),
  tryCreateSong: (song: any, jwt?: string | null) => ax.put("/songs", song, { headers: H(jwt) }),
  deleteSong: (song: any, jwt?: string | null) =>
    ax.delete("/songs", { data: { ...song }, headers: H(jwt) }),
  deleteMentions: (id: string, channelIds: string[], jwt?: string | null) =>
    ax.delete(`videos/${id}/mentions`, { data: { channel_ids: channelIds }, headers: H(jwt) }),
  addMention: (id: string, channel_id: string, jwt?: string | null) =>
    ax.post(`videos/${id}/mentions`, { channel_id }, { headers: H(jwt) }),
  songListByVideo: (channel_id: string, video_id: string, allowCache = true) =>
    ax.post(`/songs/latest?c=${allowCache ? "_" : Math.floor(Math.random() * 100)}`, {
      channel_id,
      video_id,
      limit: 1000,
    }),
  ...
  deletePlaylist: (id: string | number, jwt: string) =>
    ax.delete(`/playlist/${id}`, { headers: H(jwt) }),
  reportVideo: (id: string, body: any[], jwt: string) =>
    ax.post(`/reports/video/${id}`, body, { headers: H(jwt) }),
  ...
  getPlaylistState: (id: string, jwt?: string | null) =>
    ax.get(`/video-playlist/${id}`, { headers: H(jwt) }),
  addVideoToPlaylist: (id: string, plId: string | number, jwt?: string | null) =>
    ax.put(`/video-playlist/${plId}/${id}`, null, { headers: H(jwt) }),
  deleteVideoFromPlaylist: (id: string, plId: string | number, jwt?: string | null) =>
    ax.delete(`/video-playlist/${plId}/${id}`, { headers: H(jwt) }),
  postTL: (a: any) =>
    ax.post(
      `/videos/${a.videoId}/chats?${qs({ lang: a.lang, custom_video_id: a.custom_video_id })}`,
      a.body,
      { headers: H(a.jwt) },
    ),
  ...
  postTLLog(a: {
    ...
    return ax.post(
      `/videos/${a.videoId}/scripteditor/log?${qs({ lang: a.lang, custom_video_id: a.custom_video_id })}`,
      a.body,
      { headers },
    );
  },
  postChangeLink: (a: { jwt?: string; body: any }) =>
    ax.post("/tlutil/updateCustomLink", a.body, { headers: H(a.jwt) }),
  checkMchadMigrate: (Room: string, Pass: string) =>
    ax.post("/tlutil/migrate/check", { Room, Pass }),
  claimMchadMigrate: (jwt: string, Room: string, Pass: string) =>
    ax.post("/tlutil/migrate/claim", { Room, Pass }, { headers: H(jwt) }),
  relayBotLogin: (code: string, mode: any) =>
    axios.post(`${MCHATX}/holodexproxybot/user`, { code, mode }),
  relayBotCheckBotPresence: (ids: string[]) =>
    axios.post(`${MCHATX}/holodexproxybot/guild`, { ids }),
  relayBotGetChannels: (guild: string) =>
    axios.post(`${MCHATX}/holodexproxybot/channel`, { guild }),
  relayBotGetSettingChannel: (channel: string) =>
    axios.post(`${MCHATX}/holodexproxybot/data`, { channel }),
  relayBotSubmitData: (Address: string, SubChannel: any) =>
    axios.post(`${MCHATX}/holodexproxybot/submit`, { Address, SubChannel }),
  relayBotTrigger: (Address: string, mode: any, link: string, lang: string) =>
    axios.post(`${MCHATX}/holodexproxybot/trigger`, { Address, mode, link, lang }),
