...
const presetsFor = (s: Store, mobile: boolean) =>
  mobile
    ? s.decodedMobilePresets
    : (decodedAuto(s) as DecodedPreset[]).concat(s.decodedDesktopPresets as DecodedPreset[]);
...
function isPreset(s: Store, layout = s.layout, mobile = false) {
  ...
  return cmp.some((p) =>
    p.layout.every(
      (c, i) =>
        c.x === sorted[i].x && c.y === sorted[i].y && c.w === sorted[i].w && c.h === sorted[i].h,
    ),
  );
}

export function setMultiview(
  s: Store,
  {
    layout,
    content,
  ...
  if (mergeContent) {
    ...
    let vi = 0;
    const curVids = s.layout.filter(
      ({ i }) => s.layoutContent[i]?.type === "video" && i !== excludeId,
    );
    ...
    cl.filter((i) => !cc[i.i]).forEach((i) => {
      if (vi < s.activeVideos.length && curVids[vi]) {
        ...
        i.i = k;
        if (fillVideo && !filled) {
          merge[k] = { id: fillVideo.id, type: "video", video: fillVideo };
          filled = true;
        }
      }
    });

    const chats = Object.values(s.layoutContent)
...
export function addVideoAutoLayout(
  s: Store,
  video: any,
  mobile: boolean,
  onConflict: (l: DecodedPreset) => void,
) {
  const next = findForCount(
    mobile ? s.decodedMobilePresets : decodedAuto(s),
    s.activeVideos.length + 1,
  );
  if (!next) return;
...
export function deleteVideoAutoLayout(s: Store, id: string, mobile: boolean) {
  if (!(isPreset(s, s.layout, mobile) && s.layoutContent[id]?.type !== "chat")) {
    s.removeLayoutItem(id);
    return;
  }
  if (s.nonChatCellCount === 1) {
    s.reset();
    return;
  }
  const next = (mobile ? s.decodedMobilePresets : decodedAuto(s)).find(
    (p) => p.videoCellCount === s.nonChatCellCount - 1,
  );
  if (!next) {
    s.removeLayoutItem(id);
    return;
