...
export function buildThemeCss(hex: string): string {
  ...
  const fgT    = tc(0.10, 0.025);  // both: foreground text
  const mutFgT = tc(0.25, 0.060);  // both: muted-foreground (labels)
  const bdrT   = tc(0.10, 0.028);  // light: border, input
  const secLT  = tc(0.07, 0.020);  // light: secondary / muted / accent panel
  const sidLT  = tc(0.05, 0.015);  // light: sidebar background

  // Primary – lightness clamped to readable ranges; chroma + hue unchanged.
  const lp = f(Math.max(0.28, Math.min(0.50, L)), C);
