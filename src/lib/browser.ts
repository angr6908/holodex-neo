...
type HistoryDb = {
  ...
  put: (k: string, v: unknown) => Promise<boolean>;
...
export function openUserMenu() {
  ...
  try {
    sessionStorage.setItem(USER_MENU_KEY, "1");
  } catch {}
...
export const sendTokenToExtension = (token: string | null) =>
  sendToExt({ message: MESSAGE_TYPES.TOKEN, token });
export const sendFavoritesToExtension = (favorites: any[]) =>
  sendToExt({ message: MESSAGE_TYPES.FAVORITES, favorites });
...
const getDb = () => {
  ...
  dbPromise ??= import("kv-idb")
    .then((m: any) => (m.default || m)("watch-history") as Promise<HistoryDb>)
    .catch(() => null);
  return dbPromise;
};
...
function persistWatchedSet() {
  ...
  try {
    window.localStorage?.setItem(WATCHED_IDS_KEY, JSON.stringify([...watchedCache]));
  } catch {}
}
...
export const readWatchControlsState = (): WatchControlsState =>
  readJSON(WATCH_KEY, defaultWatchControlsState);
