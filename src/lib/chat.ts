...
export function replayTimedContinuation(origin: CVPair, { top = false, seekMs = 0 }: { top?: boolean; seekMs?: number } = {}): string {
  ...
  const payload = ld(156074452, [ld(3, hdt(origin)), vt(8, 1), ld(11, vt(2, seekMs)), ld(14, vt(1, chatType)), vt(15, 1)]);
  return encodeURIComponent(u8tob64(payload));
}

const u8tob64 = (data: Uint8Array) => globalThis.btoa
...
function ld(fid: bigint | number, payload: Uint8Array[] | Uint8Array | string): Uint8Array {
  const b = typeof payload === "string" ? new TextEncoder().encode(payload) : Array.isArray(payload) ? concatu8(payload) : payload;
...
function concatu8(args: Uint8Array[]): Uint8Array {
  ...
  let offset = 0;
  for (const a of args) { out.set(a, offset); offset += a.length; }
  return out;
}

function encv(n: bigint): bigint {
  let s = 0n;
  while (n >> 7n) { s = (s << 8n) | 0x80n | (n & 0x7fn); n >>= 7n; }
  return (s << 8n) | n;
}

function hdt(tgt: CVPair): string {
  return u8tob64(concatu8([ld(1, ld(5, [ld(1, tgt.channelId), ld(2, tgt.videoId)])), ld(3, ld(48687757, ld(1, tgt.videoId))), vt(4, 1)]));
}
