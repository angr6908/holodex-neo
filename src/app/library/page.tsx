        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <DropdownMenu open={exportMenuOpen} onOpenChange={setExportMenuOpen}>
            ...
            <DropdownMenuContent align="start">
              <DropdownMenuItem onSelect={() => setInstructionsDialog(true)}><icons.YoutubeIcon className="h-4 w-4" />{t("views.library.exportYtPlaylist")}</DropdownMenuItem>
              <DropdownMenuItem onSelect={() => void downloadAsCSV()}><Table className="h-4 w-4" />{t("views.library.exportCsv")}</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <Button type="button" variant="destructive" onClick={() => setDeleteDialog(true)}>{t("views.library.deleteFromLibraryButton", { arg0: selected.length })}</Button>
          <AlertDialog open={deleteDialog} onOpenChange={setDeleteDialog}>
            <AlertDialogContent className="max-w-[290px] p-4 sm:max-w-[290px]">
              ...
              <AlertDialogFooter className="flex-row justify-end">
                ...
                <AlertDialogAction variant="destructive" onClick={deleteSelected}>{t("views.library.deleteConfirmationOK")}</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        ...
        <DialogContent className={`${app.isMobile ? "max-w-[90%]" : "max-w-[60vw]"} p-4`}>
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_auto]">
            <div>
              ...
              <Button type="button" className="mt-2 mr-2" onClick={exportSelected}>{t("views.library.createYtPlaylistButton", { arg0: selected.length })}</Button>
