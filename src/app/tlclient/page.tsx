        ...
        {modalMode === 1 ? (
          ...
          <div className="space-y-5 p-6">
            ...
            <div className="flex justify-center"><Button onClick={settingOKClick}>{t("views.tlClient.okBtn")}</Button></div>
	          ...
	          <div className="space-y-5 p-6"><h2 className="text-lg font-normal">{t("views.tlClient.loadChatPanel.title")}</h2><Input value={activeURLStream} placeholder={t("views.tlClient.loadChatPanel.inputLabel")} onChange={(event) => setActiveURLStream(event.target.value)} /><div className="flex items-center gap-3"><Button variant="ghost" onClick={() => setModalNexus(false)}>{t("views.tlClient.cancelBtn")}</Button><Button className="ml-auto" onClick={() => { loadChat(activeURLStream); setModalNexus(false); }}>{t("views.tlClient.okBtn")}</Button></div></div>
        ) : modalMode === 5 ? (
	          <div className="space-y-5 p-6"><h2 className="text-lg font-normal">{t("views.tlClient.unloadChatTitle")}</h2><div className="flex items-center gap-3"><Button variant="ghost" onClick={() => setModalNexus(false)}>{t("views.tlClient.cancelBtn")}</Button><Button className="ml-auto" onClick={() => { unloadAll(); setModalNexus(false); }}>{t("views.tlClient.okBtn")}</Button></div></div>
        ) : null}
        </DialogContent>
      </Dialog>
