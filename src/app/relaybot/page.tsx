...
export default function RelayBotPage() {
  ...
  return (
    <section className="mx-auto min-h-screen w-full max-w-[1600px] px-3 pb-10 pt-[var(--nav-total-height,120px)] sm:px-5 space-y-6">
      <header className="space-y-2"><h1 className="text-3xl font-semibold tracking-tight">{t("views.relayBot.title")}</h1><p className="max-w-3xl text-sm text-muted-foreground">{t("views.relayBot.description")}</p></header>
      {!loggedIn ? (
        ...
        <div className="grid gap-6 xl:grid-cols-[0.9fr_0.9fr_1.5fr]">
          <Card className="flex min-h-[32rem] flex-col p-4">
            <div className="border-b px-2 pb-3 text-center text-lg font-normal">{t("views.relayBot.servers")}</div>
            <ScrollArea className="mt-4 min-h-0 flex-1 pr-1"><ToggleGroup variant="outline" size="sm" value={selectedGuild >= 0 ? [String(selectedGuild)] : []} onValueChange={(value) => { if (value[0]) loadChannel(Number(value[0])); }} className="flex w-full flex-col items-stretch gap-2">{guilds.map((guild, index) => <ToggleGroupItem key={guild.id} value={String(index)} className="h-auto w-full justify-start px-3 py-2 text-left font-normal whitespace-normal">{guild.name}</ToggleGroupItem>)}</ToggleGroup></ScrollArea>
            <p className="mt-4 text-xs leading-5 text-muted-foreground">{t("views.relayBot.serverNotShown")}</p>
          </Card>
          {selectedGuildData ? (
            <Card className="flex min-h-[32rem] flex-col p-4">
              <div className="border-b px-2 pb-3 text-center text-lg font-normal">{selectedGuildData.bot ? t("views.relayBot.channelsFor", { name: selectedGuildData.name }) : selectedGuildData.name}</div>
              {!selectedGuildData.bot ? <div className="flex flex-1 flex-col items-center justify-center gap-4 px-4 text-center"><p className="text-sm text-muted-foreground">{t("views.relayBot.botMissing")}</p><Button nativeButton={false} render={<a href={botInviteLink} target="_blank" rel="noopener noreferrer" />}>{t("views.relayBot.inviteBot")}</Button></div> : <ScrollArea className="mt-4 min-h-0 flex-1 pr-1"><ToggleGroup variant="outline" size="sm" value={selectedChannel >= 0 ? [String(selectedChannel)] : []} onValueChange={(value) => { if (value[0]) loadSetting(Number(value[0])); }} className="flex w-full flex-col items-stretch gap-2">{channels.map((channel, index) => <ToggleGroupItem key={channel.id} value={String(index)} className="h-auto w-full justify-start px-3 py-2 text-left font-normal whitespace-normal">{channel.name}</ToggleGroupItem>)}</ToggleGroup></ScrollArea>}
            </Card>
          ) : null}
          {selectedChannelData ? (
            <Card className="space-y-6 p-5">
              <div><h2 className="text-lg font-normal">{t("views.relayBot.settingFor", { name: selectedChannelData.name })}</h2></div>
              ...
              <section className="space-y-3 border-t pt-6">
                <h3 className="text-sm font-normal text-muted-foreground">{t("views.relayBot.relaySubscription")}</h3>
