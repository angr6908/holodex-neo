...
export default function AddChannelPage() {
  ...
  const linkError =
    !link || type === MODIFY_EXISTING || type === DELETE
      ? ""
      : linkRule === true
        ? ""
        : String(linkRule);
  ...
  const langError =
    !(type === ADD_CLIPPER || type === MODIFY_EXISTING) || lang ? "" : t("component.form.required");
  function alertText(tType: string) {
    switch (tType) {
      case ADD_VTUBER:
  ...
  function isFormValid() {
    ...
    if ((type === ADD_VTUBER || type === ADD_CLIPPER) && linkRule !== true) return false;
    if ((type === ADD_CLIPPER || type === MODIFY_EXISTING) && !lang) return false;
    if (type === DELETE && !contact) return false;
    if (twitterRuleResult !== true) return false;
    if ((type === MODIFY_EXISTING || type === DELETE) && !channel?.id) return false;
    return true;
  }
  async function onSubmit() {
    ...
    const fields: any[] = [
      ...
      {
        name: "Channel Link",
        value: link || `https://www.youtube.com/channel/${channel.id}`,
        inline: false,
      },
    ];
    if (englishName)
      fields.push({ name: "Alternate Channel Name (optional)", value: englishName, inline: false });
    ...
    if (org || comments)
      fields.push({ name: "Comments", value: `[${org}] ${comments}`, inline: false });
    try {
        ...
        <Card className="p-6">
          ...
          {type && alertText(type) ? (
            <Alert className="mt-4">
              <AlertDescription>
                <p dangerouslySetInnerHTML={{ __html: String(alertText(type)) }} />
              </AlertDescription>
            </Alert>
          ) : null}
          <div className="mt-6 space-y-6">
            ...
            {type === MODIFY_EXISTING || type === DELETE ? (
              <div>
                <ChannelAutocomplete
                  value={channel}
                  onChange={setChannel}
              ...
              <Field data-invalid={!!linkError}>
                <FieldLabel>{t("channelRequest.ChannelURLLabel")}</FieldLabel>
                <Input
                  value={link}
                  onChange={(e) => setLink(e.target.value)}
                  placeholder={t("channelRequest.ChannelURLPlaceholder")}
                />
                <FieldDescription>{t("channelRequest.ChannelURLExample")}</FieldDescription>
                {linkError ? <FieldError>{linkError}</FieldError> : null}
              </Field>
            )}
            {type !== DELETE && type !== ADD_CLIPPER ? (
              <Field>
                <FieldLabel>{t("channelRequest.EnglishNameLabel")}</FieldLabel>
                <Input value={englishName} onChange={(e) => setEnglishName(e.target.value)} />
                <FieldDescription>{t("channelRequest.EnglishNameHint")}</FieldDescription>
              </Field>
            ) : null}
            {!(type === ADD_VTUBER || type === DELETE) ? (
              <Field data-invalid={!!langError}>
                <FieldLabel>{t("channelRequest.ChannelLanguageLabel")}</FieldLabel>
                <Select
                  value={lang || "__none__"}
                  onValueChange={(value) => setLang(value === "__none__" ? "" : value)}
                >
                ...
                {langError ? <FieldError>{langError}</FieldError> : null}
              </Field>
            ) : null}
            {type === ADD_VTUBER || type === MODIFY_EXISTING ? (
              <Field>
                <FieldLabel>{t("channelRequest.VtuberGroupLabel")}</FieldLabel>
                <Input
                  value={org}
                  onChange={(e) => setOrg(e.target.value)}
                  placeholder={t("channelRequest.VtuberGroupPlaceholder")}
                />
                <FieldDescription>{t("channelRequest.VtuberGroupHint")}</FieldDescription>
              </Field>
            ) : null}
            {type !== DELETE ? (
              <Field data-invalid={!!twitterError}>
                <FieldLabel>{t("channelRequest.TwitterHandle")}</FieldLabel>
                <Input
                  value={twitter}
                  onChange={(e) => setTwitter(e.target.value)}
                  placeholder={t("channelRequest.TwitterPlaceholder")}
                />
                <FieldDescription>{t("channelRequest.TwitterExample")}</FieldDescription>
                {twitterError ? <FieldError>{twitterError}</FieldError> : null}
              </Field>
            ) : null}
            <Field data-invalid={!!contactError}>
              <FieldLabel>{t("channelRequest.DirectContactLabel")}</FieldLabel>
              <Input
                value={contact}
                onChange={(e) => setContact(e.target.value)}
                placeholder={t("channelRequest.DirectContactPlaceholder")}
              />
              <FieldDescription>{t("channelRequest.DirectContactDisclaimer")}</FieldDescription>
