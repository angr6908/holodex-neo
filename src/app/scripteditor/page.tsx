...
function EnhancedEntry({ stext = "", className = "", onMouseDown }: { stext?: string; className?: string; onMouseDown?: React.MouseEventHandler<HTMLSpanElement> }) {
  return <span className={`mx-1 my-0.5 break-words text-center font-normal ${className}`.trim()} onMouseDown={onMouseDown}>{stext}</span>;
}
  ...
  function renderTimelineCanvas(canvas: HTMLCanvasElement | null, idx: number) {
    ...
    for (let x = 0; x / 10 < secPerBar; x += step) {
      if (secToPx <= 60 || x % 10 === 0) {
        ...
        ctx.stroke();
        ctx.fillText(formatTlRulerTimestamp(x / 10 + idx * secPerBar + barCount * secPerBar), (x * secToPx) / 10 + 5, barHeight);
      } else {
        ctx.beginPath();
        ctx.moveTo((x * secToPx) / 10, 0);
    ...
    <div className="flex h-screen max-h-screen px-3" onKeyDown={(event) => { if (event.ctrlKey && event.key.toLowerCase() === "s") { event.preventDefault(); processLog(); } }}>
      ...
      <Dialog open={modalNexus} onOpenChange={(open) => { if (open) setModalNexus(true); else if (modalMode !== 5) closeModal(); }}>
        <DialogContent className={modalMode === 7 ? "max-w-sm p-0" : "max-w-2xl p-0"}>
	        {modalMode === 1 ? <Card className="space-y-5 p-5"><div className="text-lg font-normal">{t("views.tlClient.addProfilePanel.title")}</div><Input value={addProfileNameString} placeholder={t("views.tlClient.addProfilePanel.inputLabel")} onChange={(event) => setAddProfileNameString(event.target.value)} /><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => closeModal()}>{t("views.tlClient.cancelBtn")}</Button><Button size="sm" onClick={addProfile}>{t("views.tlClient.okBtn")}</Button></div></Card> : modalMode === 2 ? <Card className="space-y-5 p-5"><div className="text-lg font-normal">{`${t("views.tlClient.removeProfileTitle")} ${profile[profileIdx]?.Name}.`}</div><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => closeModal()}>{t("views.tlClient.cancelBtn")}</Button><Button size="sm" onClick={deleteProfile}>{t("views.tlClient.okBtn")}</Button></div></Card> : modalMode === 4 ? <Card className="space-y-5 p-5"><div className="text-lg font-normal">{t("views.scriptEditor.setStartTitle")}</div><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => closeModal()}>{t("views.tlClient.cancelBtn")}</Button><Button size="sm" onClick={() => { setStartEntry(); closeModal(); }}>{t("views.tlClient.okBtn")}</Button></div></Card> : modalMode === 5 ? <Card className="space-y-5 p-5"><div className="text-lg font-normal">{t("views.tlClient.settingPanel.title")}</div><div className="text-sm text-muted-foreground">{`${t("views.watch.uploadPanel.usernameText")} : ${appStore.userdata.user?.username || ""} `}<Button type="button" variant="link" className="h-auto p-0 text-xs underline" onClick={changeUsernameClick}>{t("views.watch.uploadPanel.usernameChange")}</Button></div><Label className="flex flex-col items-stretch gap-2 text-sm font-normal leading-normal select-auto"><span>{t("views.watch.uploadPanel.tlLang")}</span><Select value={TLLang.value} onValueChange={(value) => setTLLang(TL_LANGS.find((item) => item.value === value) || TL_LANGS[0])}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{TL_LANGS.map((item) => <SelectItem key={item.value} value={item.value}>{item.text} ({item.value})</SelectItem>)}</SelectContent></Select></Label><Input value={activeURLStream} placeholder={t("views.tlClient.settingPanel.mainStreamLink")} onChange={(event) => setActiveURLStream(event.target.value)} /><div className="flex justify-center"><Button size="sm" onClick={settingOKClick}>{t("views.tlClient.okBtn")}</Button></div></Card> : modalMode === 6 ? <Card className="p-0"><ScriptEditorExportToFile entries={entries} profile={profile} title={`${appStore.userdata.user?.username || ""} - ${videoData?.title || activeURLStream || "Holodex"}`} /></Card> : modalMode === 7 ? <Card className="space-y-5 p-5"><div className="text-lg font-normal">{t("views.scriptEditor.menu.clearAll")}</div><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => closeModal()}>{t("views.tlClient.cancelBtn")}</Button><Button variant="destructive" size="sm" onClick={clearAll}>{t("views.tlClient.okBtn")}</Button></div></Card> : modalMode === 9 ? <Card className="space-y-5 p-5"><div className="text-lg font-normal">{t("views.scriptEditor.menu.timeShift")}</div><Label className="flex flex-col items-stretch gap-2 text-sm font-normal leading-normal select-auto"><span>{t("views.scriptEditor.timeShift.offset")}</span><div className="flex items-center gap-2"><Input value={offsetInput} className="flex-1" type="number" onChange={(event) => setOffsetInput(event.target.value)} /><span className="text-muted-foreground">{t("views.scriptEditor.timeShift.seconds")}</span></div></Label><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => closeModal()}>{t("views.tlClient.cancelBtn")}</Button><Button size="sm" onClick={() => { closeModal(); shiftTime(); }}>{t("views.tlClient.okBtn")}</Button></div></Card> : modalMode === 10 ? <Card className="space-y-5 p-5"><div className="text-lg font-normal">{t("views.tlManager.changeStreamLink")}</div><Input value={linkInput} placeholder={t("views.tlManager.newLink")} onChange={(event) => setLinkInput(event.target.value)} /><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => closeModal()}>{t("views.tlClient.cancelBtn")}</Button><Button size="sm" onClick={() => closeModal()}>{t("views.tlClient.okBtn")}</Button></div></Card> : null}
        </DialogContent>
      </Dialog>
    </div>
  );
