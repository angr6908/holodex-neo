...
export default function TLScriptEditorPage() {
  ...
  const timeStampEnd = selected
    ? formatTlTimestamp(selected.Time + selected.Duration)
    : "00:00:00.00";
        ...
        const filtered = data
          ...
          .map((e: any) => {
            const timestampShifted =
              e.timestamp - (videoObj?.start_actual || data?.[0]?.timestamp || 0);
        ...
        if (entry)
          processedLog.push({
            ...
            data: {
              lang: TLLangRef.current.value,
              id: entry.id,
              name: userdataRef.current.user?.username,
              timestamp: Math.floor(
                videoObj.start_actual ? videoObj.start_actual + entry.Time : entry.realTime,
              ),
              message: entry.SText,
  ...
  function rulerMouseMove(event: React.MouseEvent) {
    ...
    switch (resizeMode.current) {
      case 0: {
        if (currentEntries[selectedIdx].Duration - xChange < 300) {
          timelineActive.current = false;
          return;
        }
        if (currentEntries[selectedIdx].Time + xChange < secPerBar * barCount * 1000) {
          timelineActive.current = false;
          return;
        }
        if (
          ...
          currentEntries[selectedIdx].Time + xChange <
            currentEntries[selectedIdx - 1].Time + currentEntries[selectedIdx - 1].Duration
        ) {
      ...
      case 1: {
        ...
        if (
          selectedIdx < currentEntries.length - 1 &&
          currentEntries[selectedIdx].Time + currentEntries[selectedIdx].Duration + xChange >
            currentEntries[selectedIdx + 1].Time
        ) {
          if (currentEntries[selectedIdx + 1].Duration - xChange > 300) {
            currentEntries[selectedIdx + 1].Duration -= xChange;
            currentEntries[selectedIdx + 1].Time =
        ...
        if (currentEntries[selectedIdx].Time + xChange > 0)
          currentEntries[selectedIdx].Time += xChange;
  ...
  useEffect(() => {
    ...
    if (timerTime < timelineEntries[0].entry.Time) {
      setDisplayEntry(-1);
      return;
    }
    ...
    >
      <div className="flex h-full w-full flex-col">
        <div className="mb-2 flex min-h-[42px] flex-wrap items-center gap-2 rounded-2xl border px-2 py-2 [&>*:not(:first-child):not(:last-child)]:mx-[3px] [&>*]:normal-case">
          ...
          <Button variant="outline" size="sm" onClick={() => processLog()}>
          ...
          <Button variant="outline" size="sm" onClick={() => setImportPanelShow(true)}>
            {t("views.scriptEditor.menu.importFile")}
          </Button>
          <Button variant="outline" size="sm" onClick={continuousTime}>
            ...
            <Table className="w-full border-collapse text-sm" width="auto">
              <TableHeader className="[&_tr]:border-b-0" onClick={() => setSelectedEntry(-1)}>
                <TableRow className="border-b-0 hover:bg-transparent">
                  ...
                  <TableHead>
                    {!vidPlayer ? (
                      <div className="absolute right-[5px] top-[5px] z-[1] flex flex-row items-center gap-2">
                        <Button variant="outline" size="sm" onClick={timerTimeStop}>
                          <Square className="size-4" />
                        </Button>
                        <span>{timerPrint}</span>
                        <Button variant="outline" size="sm" onClick={timerTimeStart}>
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          <Select
                            ...
                            onValueChange={(value) => {
                              const nextProfile = Number(value);
                              setProfileIdx(nextProfile);
                              updateEntry(index, { Profile: nextProfile });
                              showProfileList();
                            }}
                          >
                        ...
                        <TableCell colSpan={2}>
                          <Input
                            ...
                            onChange={(event) => updateEntry(index, { SText: event.target.value })}
                          />
                        </TableCell>
                      </TableRow>
                      <TableRow>
            ...
            <Card className="flex h-full w-1/2 flex-col p-0">
              ...
              <div className="flex flex-row justify-center">
                {displayEntry >= 0 && displayEntry < entries.length ? (
                  <EnhancedEntry stext={entries[displayEntry].SText} />
                ) : null}
              </div>
        ...
        <div onKeyDown={handleControlKeyDown}>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            ...
            <Button size="lg" className="lg:mx-2" onClick={addEntry}>
              {t("views.tlClient.tlControl.enterBtn")}
            </Button>
            <Button variant="secondary" size="lg" onClick={() => setTLSetting((value) => !value)}>
              {TLSetting
          ...
          {TLSetting ? (
            <Card className="mt-2 space-y-5 p-5">
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                <Input
                  value={profile[profileIdx]?.Prefix || ""}
                  className="flex-1"
                  placeholder={t("views.tlClient.tlControl.prefix")}
                  onChange={(event) => updateProfile(profileIdx, { Prefix: event.target.value })}
                />
                <Input
                  value={profile[profileIdx]?.Suffix || ""}
                  className="flex-1"
                  placeholder={t("views.tlClient.tlControl.suffix")}
                  onChange={(event) => updateProfile(profileIdx, { Suffix: event.target.value })}
                />
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline" size="sm" onClick={shiftProfileUp}>
                  {t("views.tlClient.tlControl.shiftUp")}
                </Button>
                <Button variant="outline" size="sm" onClick={shiftProfileDown}>
        ...
        <DialogContent className={modalMode === 7 ? "max-w-sm p-0" : "max-w-2xl p-0"}>
          {modalMode === 1 ? (
            <Card className="space-y-5 p-5">
              <div className="text-lg font-normal">{t("views.tlClient.addProfilePanel.title")}</div>
              <Input
            ...
            <Card className="space-y-5 p-5">
              <div className="text-lg font-normal">{`${t("views.tlClient.removeProfileTitle")} ${profile[profileIdx]?.Name}.`}</div>
              <div className="flex justify-end gap-2">
                <Button variant="outline" size="sm" onClick={() => closeModal()}>
                  {t("views.tlClient.cancelBtn")}
                </Button>
                <Button size="sm" onClick={deleteProfile}>
                  {t("views.tlClient.okBtn")}
                </Button>
              </div>
            </Card>
            ...
            <Card className="space-y-5 p-5">
              <div className="text-lg font-normal">{t("views.scriptEditor.setStartTitle")}</div>
              <div className="flex justify-end gap-2">
                <Button variant="outline" size="sm" onClick={() => closeModal()}>
                  {t("views.tlClient.cancelBtn")}
                </Button>
            ...
            <Card className="space-y-5 p-5">
              ...
              <div className="text-sm text-muted-foreground">
                {`${t("views.watch.uploadPanel.usernameText")} : ${appStore.userdata.user?.username || ""} `}
                <Button
                  type="button"
                  variant="link"
              ...
              <Label className="flex flex-col items-stretch gap-2 text-sm font-normal leading-normal select-auto">
                ...
                <Select
                  value={TLLang.value}
                  onValueChange={(value) =>
                    setTLLang(TL_LANGS.find((item) => item.value === value) || TL_LANGS[0])
                  }
                >
                  <SelectTrigger className="w-full">
                    <SelectValue />
              ...
              <div className="flex justify-center">
                <Button size="sm" onClick={settingOKClick}>
                  {t("views.tlClient.okBtn")}
                </Button>
              </div>
            </Card>
            ...
            <Card className="space-y-5 p-5">
              <div className="text-lg font-normal">{t("views.scriptEditor.menu.clearAll")}</div>
              <div className="flex justify-end gap-2">
                <Button variant="outline" size="sm" onClick={() => closeModal()}>
                  {t("views.tlClient.cancelBtn")}
                </Button>
            ...
            <Card className="space-y-5 p-5">
              <div className="text-lg font-normal">{t("views.scriptEditor.menu.timeShift")}</div>
              <Label className="flex flex-col items-stretch gap-2 text-sm font-normal leading-normal select-auto">
            ...
            <Card className="space-y-5 p-5">
              <div className="text-lg font-normal">{t("views.tlManager.changeStreamLink")}</div>
              <Input
                value={linkInput}
