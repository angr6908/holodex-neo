...
const clean = (v: unknown): v is string => typeof v === "string" && v.trim().length > 0 && v.trim().length <= 60;
const miss = { live_viewers: null, isLive: false };

export async function POST(request: Request) {
  const body = await request.json().catch(() => ({}) as any);
  const raw: unknown[] = Array.isArray(body?.logins) ? body.logins : body?.login != null ? [body.login] : [];
  ...
  const out = Object.fromEntries(logins.map((l) => {
    const n = counts[l];
