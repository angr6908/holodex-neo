...
export default function AddPlaceholderPage() {
  ...
  const token = useMemo(() => {
    try {
      return tokenRaw ? jwtDecode<any>(tokenRaw) : null;
    } catch {
      return null;
    }
  }, [tokenRaw]);
  ...
  const credits = isEditor
    ? { editor: { name: creditName, user: app.userdata.user?.id } }
    : token
      ? { discord: { name: token.name, link: token.link, user: token.user } }
      : null;
  const videoObj = {
    title: videoTitle || "Example Title",
    placeholderType: placeholderType || "scheduled-yt-stream",
    channel: channel || {
    ...
    type: "placeholder",
    status: "upcoming",
    start_scheduled: availableAt || dayjs().toISOString(),
    available_at: availableAt || dayjs().toISOString(),
    credits: credits || { discord: { user: "Discord User", link: "jctkgHBt4b" } },
    certainty,
    link: sourceUrl,
  };
  const validUrl = (value: string) => /^https?:\/\/[\w-]+(\.[\w-]+)+\.?(\/\S*)?/.test(value);
  const validation = {
    creditName: isEditor && !creditName ? t("component.form.required") : "",
    channel: channel?.id ? "" : t("component.form.required"),
    videoTitle: videoTitle ? "" : t("component.form.required"),
    sourceUrl: !sourceUrl
      ? t("component.form.required")
      : validUrl(sourceUrl)
        ? ""
        : t("component.form.invalidUrl"),
    thumbnail: !thumbnail ? "" : validUrl(thumbnail) ? "" : t("component.form.invalidUrl"),
    placeholderType: placeholderType ? "" : t("component.form.required"),
    certainty: certainty ? "" : t("component.form.required"),
    liveDate: liveDate ? "" : t("component.form.required"),
    liveTime: !liveTime
      ? t("component.form.required")
      : availableAt
        ? ""
        : t("component.form.invalidTime"),
    duration: Number(duration) > 0 ? "" : t("component.form.required"),
  ...
  async function onSubmit() {
    ...
    const titlePayload: any = {
      name: videoTitle,
      ...(videoTitleJP && { jp_name: videoTitleJP }),
      link: sourceUrl,
      ...(thumbnail && { thumbnail }),
      placeholderType,
      certainty,
  ...
  return (
    <section className="mx-auto min-h-screen w-full max-w-[1600px] px-3 pb-10 pt-[var(--nav-total-height,120px)] sm:px-5 space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(16rem,22rem)_minmax(0,1fr)]">
        ...
        <div className="space-y-4">
          {!isEditor && token && !expired ? (
            <Alert>
              <AlertDescription>
                {t("component.form.placeholder.editingAs", {
                  user: token.user,
          ...
          <Card className="p-6">
            ...
            <div className="mt-6 space-y-5">
              {isEditor ? (
                <Field data-invalid={!!validation.creditName}>
                  <FieldLabel>{t("component.form.placeholder.editorCreditName")}</FieldLabel>
                  <Input value={creditName} onChange={(e) => setCreditName(e.target.value)} />
                  <FieldDescription>
                    {t("component.form.placeholder.editorCreditHint")}
                  </FieldDescription>
                  {validation.creditName ? <FieldError>{validation.creditName}</FieldError> : null}
                </Field>
              ) : null}
              {tab === 1 ? (
                <div className="space-y-4">
                  <Field>
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <Button variant="secondary" onClick={() => loadExistingPlaceholder(id)}>
                  ...
                  {!id ? (
                    <VideoSelector
                      onVideoClicked={(video: any) => {
                        setId(video.id);
                        void loadExistingPlaceholder(video.id);
                      }}
                    />
                  ) : null}
                </div>
              ) : null}
              <Field data-invalid={!!validation.channel}>
                <FieldLabel>{t("component.form.channel")}</FieldLabel>
                <ChannelAutocomplete
                  value={channel}
                  onChange={setChannel}
                  label={t("component.form.channel")}
                />
                {validation.channel ? <FieldError>{validation.channel}</FieldError> : null}
              </Field>
              <Field data-invalid={!!validation.videoTitle}>
                <FieldLabel>{t("component.form.placeholder.videoTitle")}</FieldLabel>
                <Input value={videoTitle} onChange={(e) => setVideoTitle(e.target.value)} />
                {validation.videoTitle ? <FieldError>{validation.videoTitle}</FieldError> : null}
              </Field>
              <Field>
                <FieldLabel>{t("component.form.placeholder.japaneseVideoTitle")}</FieldLabel>
                <Input value={videoTitleJP} onChange={(e) => setVideoTitleJP(e.target.value)} />
              </Field>
              <Field data-invalid={!!validation.sourceUrl}>
                <FieldLabel>{t("component.form.placeholder.sourceLink")}</FieldLabel>
                <Input
                  value={sourceUrl}
                ...
                />
                <FieldDescription>
                  {t("component.form.placeholder.sourceLinkHint")}
                </FieldDescription>
                {validation.sourceUrl ? <FieldError>{validation.sourceUrl}</FieldError> : null}
              </Field>
              <Field data-invalid={!!validation.thumbnail}>
                <FieldLabel>{t("component.form.placeholder.thumbnailImage")}</FieldLabel>
                <Input
                  value={thumbnail}
                ...
                />
                {validation.thumbnail ? <FieldError>{validation.thumbnail}</FieldError> : null}
              </Field>
              <div className="grid gap-4 md:grid-cols-2">
                <Field data-invalid={!!validation.placeholderType}>
                  <FieldLabel>{t("component.form.placeholder.eventType")}</FieldLabel>
                  <Select value={placeholderType} onValueChange={setPlaceholderType}>
                    <SelectTrigger className="w-full">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {placeholderTypes.map((item) => (
                  ...
                  {validation.placeholderType ? (
                    <FieldError>{validation.placeholderType}</FieldError>
                  ) : null}
                </Field>
                <Field data-invalid={!!validation.certainty}>
                  <FieldLabel>{t("component.form.placeholder.certainty")}</FieldLabel>
                  <Select value={certainty} onValueChange={setCertainty}>
                    <SelectTrigger className="w-full">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {certaintyChoices.map((item) => (
                        <SelectItem key={item.value} value={item.value}>
                          {item.text}
                        </SelectItem>
                      ))}
                  ...
                  {validation.certainty ? <FieldError>{validation.certainty}</FieldError> : null}
                </Field>
              </div>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <Field>
                  <FieldLabel>{t("component.form.placeholder.timezone")}</FieldLabel>
                ...
                <Field data-invalid={!!validation.liveDate}>
                  <FieldLabel>{t("component.form.placeholder.date")}</FieldLabel>
                  <div className="flex gap-2">
                    <Button variant="secondary" size="icon" onClick={() => changeDate(-1, "day")}>
                  ...
                  {validation.liveDate ? <FieldError>{validation.liveDate}</FieldError> : null}
                </Field>
                <Field data-invalid={!!validation.liveTime}>
                  <FieldLabel>{t("component.form.placeholder.time")}</FieldLabel>
                  <div className="flex gap-2">
                    <Button variant="secondary" size="icon" onClick={() => changeDate(-1, "hour")}>
                      <MinusSquare className="h-4 w-4" />
                    </Button>
                    <Input
                      value={liveTime}
                  ...
                  {validation.liveTime ? <FieldError>{validation.liveTime}</FieldError> : null}
                </Field>
                <Field data-invalid={!!validation.duration}>
                  <FieldLabel>{t("component.form.placeholder.duration")}</FieldLabel>
                  <Input
                    value={duration}
                  ...
                  />
                  <FieldDescription>
                    {t("component.form.placeholder.durationHint")}
                  </FieldDescription>
                  {validation.duration ? <FieldError>{validation.duration}</FieldError> : null}
                </Field>
              </div>
