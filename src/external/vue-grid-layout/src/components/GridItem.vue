...
export default defineComponent({
    ...
    watch: {
        ...
        "layout.margin": function (margin: any) {
            if (!margin || (margin[0] == this.margin[0] && margin[1] == this.margin[1])) {
                return;
            }
            this.margin = margin.map((m: any) => Number(m));
            this.createStyle();
            this.emitContainerResized();
        },
    },
    ...
    methods: {
        ...
        teardownGridItem: function () {
            ...
            this.resizeEventSet = false;
            this.pendingInteractInit = false;
        },
        getInteractObj: function () {
            const item = this.$refs.item;
            if (!item) {
                if (!this.pendingInteractInit) {
                    this.pendingInteractInit = true;
                    this.$nextTick(() => {
                        this.pendingInteractInit = false;
                        if (this.$refs.item) {
            ...
            if (this.interactObj && this.interactTarget !== item) {
                this.interactObj.unset();
                this.interactObj = null;
                this.dragEventSet = false;
                this.resizeEventSet = false;
            ...
            if (this.interactObj === null || this.interactObj === undefined) {
                this.interactObj = interact(item);
            ...
            return this.interactObj;
        },
        tryMakeDraggable: function () {
            const self = this;
            const interactObj = this.getInteractObj();
        ...
        autoSize: function () {
            ...
            let newSize = this.$el.children[0].getBoundingClientRect();
