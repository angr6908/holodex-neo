...
export function AddChannelPage() {
  ...
  async function onSubmit() {
    ...
    if (type === ADD_VTUBER || type === ADD_CLIPPER) { const matches = [...link.matchAll(/(?:https?:\/\/)(?:www\.)?youtu(?:be\.com\/)(?:channel\/|@)([\w\-_]*)/gi)]; let id = matches?.[0]?.[1]; handle = link.includes("@"); id = handle ? `@${id?.toLowerCase()}` : id; try { const exists = id && (await api.channel(id)); if (exists?.data?.id) { router.push(`/channel/${exists.data.id}`); return; } } catch {} }
