      ...
      <div class="w-full max-w-sm">
        <UiCard class-name="overflow-hidden border-[color:var(--color-border)] bg-[color:var(--color-card)] p-0">
          <div class="px-6 py-7 text-center">
            ...
            <div class="space-y-3">
              <!-- Google: GIS on allowed hosts, disabled otherwise -->
              ...
              <div v-if="showManualOAuth" class="space-y-2 rounded-lg border border-[color:var(--color-border)] bg-[color:var(--color-base)] p-3 text-left">
      ...
      <div class="grid divide-y divide-[color:var(--color-border)] lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] lg:divide-x lg:divide-y-0">
        <div class="divide-y divide-[color:var(--color-border)]">
          <div class="space-y-3 px-5 py-5 sm:px-6 sm:py-6">
            ...
            <div class="grid max-w-md gap-2 sm:grid-cols-2">
              <div class="login-status-chip">
                ...
                <UiButton
                  ...
                  @click="allowedOAuthHost ? triggerGoogleLink() : undefined"
                  :disabled="!allowedOAuthHost"
                  :title="allowedOAuthHost ? '' : 'Google link is only available on localhost or holodex.net'"
                  :style="allowedOAuthHost ? '' : 'opacity: 0.4; cursor: not-allowed;'"
                >
                  Unlinked
                </UiButton>
              </div>
            ...
            <div v-if="allowedOAuthHost && !userdata.user.google_id" class="login-google-proxy" aria-hidden="true">
...
function loginGoogleManual() {
  // Intentionally empty — Google manual login is not supported
  // due to redirect_uri_mismatch. Google sign-in requires localhost or holodex.net.
}
