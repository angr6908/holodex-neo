...
export function VideoEditMentions({ video }: { video: any }) {
  ...
  function updateMentions() {
    api
      ...
      .then(({ data }: any) => setMentions(data))
      .catch(console.error);
  }
  ...
  function deleteMention(channelId: string) {
    api
      .deleteMentions(video.id, [channelId], app.userdata.jwt)
      .then(({ data }: any) => {
        if (!data) return;
  ...
  function addMention(channelId: string) {
    api
      .addMention(video.id, channelId, app.userdata.jwt)
      .then(({ data }: any) => {
        if (!data) return;
        flashAlert(
          "success",
          t("views.editor.channelMentions.addSuccess", {
            channel: channelDisplayName(selectedChannel, useEnglishName),
          }),
        );
        updateMentions();
      })
