...
function RelativeTimestampEditor({ value, test, upTo = false, onInput, onSeekTo }: { value: number; test: number; upTo?: boolean; onInput?: (value: number) => void; onSeekTo?: (value: number) => void }) {
  ...
  return <div className="group mb-4 -mx-2.5">
    <div ref={tl} className="relative cursor-pointer pt-[25px]" onClick={tryPlay}><Progress className="mx-2 mb-1.5 mt-5 h-2 [&>[data-slot=progress-indicator]]:bg-primary" value={((Number(test) - min) * 100.0) / (max - min)} /><div className="pointer-events-none absolute left-1/2 top-1 block h-5 -translate-x-1/2 border-l-2 border-border pl-1 text-[9px] opacity-0 group-hover:opacity-100">{t("editor.music.playFromHere")}</div></div>
	    <div className="relative mb-2"><Slider className="mt-3" value={[newVal]} min={min} max={max} step={1} onValueChange={(next) => { const v = Array.isArray(next) ? next[0] ?? value : next; setNewVal(v); onInput?.(v); }} /><div className="mt-1 text-center text-xs text-muted-foreground">{newVal === value ? formatDuration(value * 1000) : `${(newVal - value) > 0 ? "+" : ""}${newVal - value}s`}</div></div>
...
export const VideoEditSongs = forwardRef<VideoEditSongsHandle, VideoEditSongsProps>(function VideoEditSongs({ id, video, currentTime = 0, onTimeJump }, ref) {
  ...
  return <div id={id} className="space-y-5">
    <div className="flex items-center gap-3">
      ...
      <Separator className="flex-1" />
      <Button type="button" variant="ghost" size="sm" onClick={() => { setHelpOpen(true); mountTwitter(); }}>
    ...
    <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
      <DialogContent>
        <blockquote className="twitter-tweet">
          <p lang="en" dir="ltr">Easily create Music entries on Holodex, coming soon! 🎵🎶 <a href="https://t.co/1KJXYDcJjo">pic.twitter.com/1KJXYDcJjo</a></p>&mdash; Holodex (@holodex) <a href="https://twitter.com/holodex/status/1371290072058785797?ref_src=twsrc%5Etfw">March 15, 2021</a>
        </blockquote>
      </DialogContent>
    </Dialog>
    <div className="grid gap-3 md:grid-cols-12">
      ...
      <div className="space-y-2 md:col-span-6">
        ...
        <Input value={current.name} onChange={(e) => setCurrent((c: any) => ({ ...c, name: e.target.value }))} placeholder={t("editor.music.trackNameInput")} />
      </div>
      <div className="space-y-2 md:col-span-6">
        <Label>{t("editor.music.originalArtistInput")}</Label>
        <Input value={current.original_artist} onChange={(e) => setCurrent((c: any) => ({ ...c, original_artist: e.target.value }))} placeholder={t("editor.music.originalArtistInput")} />
      </div>
      <div className="md:col-span-6">
        <div className="flex items-start gap-2">
          <Button type="button" variant="secondary" title={t("editor.music.setToCurrentTime", { arg0: secondsToHuman(currentTime) })} onClick={() => setStartInput(secondsToHuman(currentTime))}>
            <Gauge className="h-4 w-4 rotate-90" />
            {formatDuration(currentTime * 1000)}
          </Button>
          <Input value={currentStartTime} placeholder="12:31" aria-invalid={!startTimeRegex.test(currentStartTime)} onChange={(e) => setStartInput(e.target.value)} />
        </div>
        <RelativeTimestampEditor value={Number(current.start)} test={currentTime} onInput={(x) => { setCurrent((c: any) => ({ ...c, start: x })); setCurrentStartTimeInput(secondsToHuman(x)); onTimeJump?.(x, true); }} onSeekTo={(x) => onTimeJump?.(x, true)} />
      </div>
      <div className="md:col-span-6">
        <div className="flex items-start gap-2">
          <Button type="button" variant="secondary" disabled={currentTime < current.start + 10} title={t("editor.music.setToCurrentTime", { arg0: secondsToHuman(currentTime) })} onClick={() => { setEndInput(`${currentTime - current.start}`); onTimeJump?.(currentTime - 3, true, false, currentTime); }}>
            <Gauge className="h-4 w-4 rotate-90" />
            {formatDuration(currentTime * 1000)}
          </Button>
          {current.song?.trackTimeMillis ? <Button type="button" variant="secondary" title={t("editor.music.inheritItunesMusic", { arg0: `+${Math.ceil(current.song.trackTimeMillis / 1000)}` })} onClick={() => { setEndInput(`+${Math.ceil(current.song.trackTimeMillis / 1000)}`); onTimeJump?.(current.start + current.song.trackTimeMillis / 1000 - 3, true, false, current.start + current.song.trackTimeMillis / 1000); }}>
            <Plus className="h-4 w-4" />
            {formatDuration(current.start * 1000 + current.song.trackTimeMillis)}
          </Button> : null}
          <Input value={currentEndTime} placeholder="312" aria-invalid={!endTimeRegex.test(currentEndTime)} onChange={(e) => setEndInput(e.target.value)} />
        </div>
        <RelativeTimestampEditor value={Number(current.end)} upTo test={currentTime} onInput={(x) => { setCurrent((c: any) => ({ ...c, end: x })); onTimeJump?.(x - 3, true, false, x); }} onSeekTo={(x) => onTimeJump?.(x, true)} />
      </div>
      <div className="md:col-span-8">
        <Button type="button" className="w-full" disabled={!canSave || !priviledgeSufficient} onClick={addSong}>{addOrUpdate}</Button>
      </div>
      <div className="md:col-span-1">
        <Button type="button" variant="destructive" className="w-full" onClick={reset}><RotateCcw className="size-5" /></Button>
      </div>
      <div className="md:col-span-3">
        <Button nativeButton={false} render={<a href={current.amUrl || "#"} rel="norefferer" target="_blank" />} variant="secondary" disabled={!current.amUrl} className="justify-start whitespace-normal text-left">
          <img src="https://apple-resources.s3.amazonaws.com/medusa/production/images/5f600674c4f022000191d6c4/en-us-large@1x.png" className="h-6 w-6 rounded-sm object-cover" alt="" />
          <span>{t("editor.music.listenOnAppleMusic")}</span>
        </Button>
      </div>
      {!canSave && !priviledgeSufficient ? <Alert variant="destructive" className="md:col-span-12"><AlertDescription dangerouslySetInnerHTML={{ __html: t.raw("editor.music.permission") }} /></Alert> : null}
    </div>
    <div className="flex items-center gap-3">
      <Separator className="flex-1" />
      <span className="text-sm text-muted-foreground">{t("editor.music.titles.songList", { arg0: video.title })}</span>
      <Separator className="flex-1" />
    </div>
    <ScrollArea className="max-h-[45vh] min-h-[30vh]">
      <div className="space-y-2">
        {songList.map((song) => <SongItem key={song.name} song={song} detailed hoverIcon={icons.Pencil} artworkHoverIcon={icons.Play} onRemove={removeSong} onPlay={(x: any) => { onTimeJump?.(x.start); setCurrent(structuredClone(x)); setCurrentStartTimeInput(secondsToHuman(x.start)); }} onPlayNow={(x: any) => onTimeJump?.(x.start, true)} />)}
      </div>
    </ScrollArea>
  </div>;
});
