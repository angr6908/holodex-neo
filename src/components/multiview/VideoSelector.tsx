...
export function VideoSelector({
  ...
  const [selectedOrg, setSelectedOrg] = useState<any>(() => {
    ...
    if (selectedName)
      return (
        (app.orgs || []).find((org: any) => org.name === selectedName) ||
        app.currentOrg || { name: selectedName, short: selectedName.slice(0, 4) }
      );
  ...
  const shouldHideCollabs =
    (selectedOrg?.name === "Favorites" || isRealOrg || isMultiOrg) &&
    app.settings.hideCollabStreams;
  const connectedListOrgTargets = isMultiOrg
    ? selectedOrgNames
    : isRealOrg
      ? [selectedOrg.name]
      : null;
  ...
  const baseFilteredLive = useMemo(() => {
    ...
    const isTwitchPlaceholder = (v: any) =>
      v.type === "placeholder" && v.link?.includes("twitch.tv");
  ...
  function loadSelection(panel = selectedOrg) {
    ...
    if (panel?.name === "Favorites") {
      ...
      setIsLoading(true);
      api
        .favoritesLive({ includePlaceholder: true }, app.userdata.jwt)
        .then((data: any[]) => {
    ...
    api
      .allLive(targets, { type: "placeholder,stream", include: "mentions,channels" })
      .then((videos: any[]) => {
        if (requestId !== loadRequestId.current) return;
        const merged: any[] = [];
  ...
  function getHomeOrgSelection() {
    ...
    if (selectedName)
      return (
        (app.orgs || []).find((org: any) => org.name === selectedName) ||
        (app.currentOrg?.name === selectedName ? app.currentOrg : null) || {
          name: selectedName,
          short: selectedName.slice(0, 4),
        }
      );
  ...
  function handleOrgApply(names: string[]) {
    ...
    const fallback = (app.orgs || []).find((org: any) => org.name === DEFAULT_ORG) || {
      name: DEFAULT_ORG,
      short: "Holo",
    };
    if (unique.length === 0) {
      ...
      app.setCurrentOrg(
        (app.orgs || []).find((org: any) => org.name === ALL_VTUBERS_ORG) || allVtubersTab,
      );
      handlePicker(panel);
      loadSelection(panel);
  ...
  function handleInlineUrl(event: React.FormEvent) {
    event.preventDefault();
    if (!submitVideoUrl(inlineUrl, multiview, onVideoClicked)) {
      ...
      <>
        <div className="mb-3 flex items-center gap-1.5 border-b pb-3">
          <HomeOrgMultiSelect
            buttonVariant="ghost"
            manualApply
            selectedNamesOverride={selectedHomeOrgsForPicker}
          ...
          <Button
            ...
            onClick={() => handlePicker(playlistTab)}
          >
            <ListPlus />
          </Button>
          <form
            className="relative flex h-8 min-w-[8rem] flex-1 items-center"
            onSubmit={handleInlineUrl}
          >
            <Input
              value={inlineUrl}
              type="url"
          ...
          {isUrl ? (
            ...
            <div className="px-3 py-6 text-center">
              ...
              <div className="mt-4">
                <Button
                  variant="ghost"
                  onClick={() => leaveMultiviewForHome({ openLogin: !isLoggedIn })}
                >
              ...
              {selectedOrg?.name !== "Playlist" ? (
                <div className="mb-3 flex flex-wrap items-center gap-2 px-1">
                  <ToggleGroup
                    value={[String(tab)]}
                    onValueChange={(value) => {
                      if (value[0]) setTab(Number(value[0]));
                    }}
                  >
            ...
            <DropdownMenu open={showPlaylistMenu} onOpenChange={setShowPlaylistMenu}>
              ...
              <DropdownMenuContent align="start" sideOffset={8} className="min-w-[13rem]">
                <DropdownMenuItem
                  ...
                  onSelect={() => selectQuickTab(playlistTab)}
                >
                ...
                >
                  {topFilteredLive.map((video, index) => (
                    <div
                      ...
                      title={video.title}
                      draggable
                      onDragStart={(event) => dragVideo(event, video)}
                    >
