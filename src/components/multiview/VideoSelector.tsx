...
function MvUrlInput({ className = "", onSuccess }: { className?: string; onSuccess?: (content: any) => void }) {
  ...
  return (
    <div className={`flex min-w-8 items-center gap-1 has-[form]:min-w-0 has-[form]:max-w-[280px] ${className}`}>
      {!expanded ? (
        <Button type="button" size="icon-sm" variant="secondary" className="shrink-0" title={t("views.multiview.video.addUrl")} onClick={() => setExpanded(true)}><Link className="size-5" /></Button>
      ) : (
        <form className="flex items-center gap-1 animate-in fade-in slide-in-from-right-1 duration-150" onSubmit={handleSubmit}>
          <Button type="button" size="icon-sm" variant="secondary" className="shrink-0" title={t("component.common.collapse")} onClick={collapse}><ChevronLeft className="size-5" /></Button>
          <ButtonGroup className="h-8 min-w-0 flex-1">
            ...
            {url ? <Button type="submit" size="icon-sm" variant={hasError ? "outline" : "default"} className="shrink-0" title={t("component.common.confirm")}><Check className="size-5" /></Button> : null}
          </ButtonGroup>
        </form>
      )}
    </div>
...
export function VideoSelector({ horizontal = false, isActive = true, compact = false, hidePlaceholder, hideMissing, activeVideos: activeVideosOverride, onVideoClicked }: { horizontal?: boolean; isActive?: boolean; compact?: boolean; hidePlaceholder?: boolean; hideMissing?: boolean; activeVideos?: any[]; onVideoClicked?: (video: any) => void }) {
    ...
    return (
      <Card className="flex max-h-[min(80dvh,calc(100dvh-2rem))] min-h-0 flex-col p-3">
        <div className="mb-3 flex items-center gap-1.5 border-b pb-3">
          <HomeOrgMultiSelect className="min-w-0 flex-1" manualApply selectedNamesOverride={selectedHomeOrgsForPicker} fallbackSelection={[hololiveName]} buttonClass="w-full justify-between" onApply={handleOrgApply} />
	          <Button type="button" size="icon-sm" variant={selectedOrg?.name === "Favorites" ? "default" : "secondary"} className="shrink-0" title={t("component.mainNav.favorites")} onClick={() => handlePicker(favTab)}>
            <icons.Heart className="h-4 w-4" />
          </Button>
	          <Button type="button" size="icon-sm" variant={selectedOrg?.name === "Playlist" ? "default" : "secondary"} className="shrink-0" title={t("component.mainNav.playlist")} onClick={() => handlePicker(playlistTab)}>
          ...
          <form className="relative flex h-8 min-w-[8rem] flex-1 items-center" onSubmit={handleInlineUrl}>
            ...
            {inlineUrl ? <Button type="submit" size="icon-sm" variant={inlineUrlError ? "outline" : "default"} className="shrink-0" title={t("views.multiview.video.addUrlShort")}><icons.Check className="h-4 w-4" /></Button> : null}
          </form>
          {!isUrl ? <Button type="button" size="icon-sm" variant="secondary" className={cn("shrink-0", isLoading && "animate-spin")} title={t("component.apiError.refresh")} onClick={() => loadSelection()}><icons.RefreshCw className="size-5" /></Button> : null}
        </div>
        <div ref={container} className="min-h-0 flex-1 overflow-y-auto px-1 sm:px-2">
          {isUrl ? (
            <>
              <div className="px-2 py-1 text-sm font-semibold text-muted-foreground">{t("views.multiview.video.addCustomVideo")}</div>
            ...
            <div className="px-3 py-6 text-center">
              ...
              <Button className="mt-4" onClick={() => leaveMultiviewForHome({ openLogin: !isLoggedIn })}>{isLoggedIn ? t("views.favorites.manageFavorites") : t("component.mainNav.login")}</Button>
            </div>
          ) : (
            <>
  ...
  return (
    <div className="flex w-full min-w-0 items-center gap-1">
      {!compact ? (
        <>
          <HomeOrgMultiSelect className="shrink-0 self-center" manualApply iconOnly selectedNamesOverride={selectedHomeOrgsForPicker} fallbackSelection={[hololiveName]} buttonClass="overflow-hidden" onApply={handleOrgApply} />
	          <Button type="button" size="icon-sm" variant={selectedOrg?.name === "Favorites" ? "default" : "secondary"} className="shrink-0" title={t("component.mainNav.favorites")} onClick={() => selectQuickTab(favTab)}>
            <icons.Heart className="h-4 w-4" />
          </Button>
          <DropdownMenu open={showPlaylistMenu} onOpenChange={setShowPlaylistMenu}>
            <DropdownMenuTrigger
		              render={<Button type="button" size="icon-sm" variant={selectedOrg?.name === "Playlist" ? "default" : "secondary"} className="shrink-0" title={t("component.mainNav.playlist")} />}
            >
              <icons.ListPlus className="h-4 w-4" />
            </DropdownMenuTrigger>
              <DropdownMenuContent align="start" sideOffset={8} className="min-w-[13rem]">
                ...
                <icons.ListPlus className="h-4 w-4 shrink-0" />
                <span>{t("component.mainNav.playlist")}</span>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <MvUrlInput className="shrink-0 self-center" onSuccess={handleVideoClick} />
          {!isUrl ? <Button type="button" size="icon-sm" variant="secondary" className={cn("shrink-0", isLoading && "animate-spin")} title={t("component.apiError.refresh")} onClick={() => loadSelection()}><icons.RefreshCw className="h-4 w-4" /></Button> : null}
        </>
      ) : null}
      {selectedOrg?.name === "Favorites" && !isLoggedIn ? (
        <div className="flex items-center gap-2 self-center text-sm text-muted-foreground"><span dangerouslySetInnerHTML={{ __html: t.raw("views.app.loginCallToAction") }} /><Button variant="ghost" size="sm" onClick={() => leaveMultiviewForHome({ openLogin: true })}>{t("component.mainNav.login")}</Button></div>
      ) : (
        <div className={cn("min-h-14 min-w-0 flex-1 overflow-visible", compact && "min-h-10")} onWheel={scrollHandler}>
          {isLoading && topFilteredLive.length === 0 ? (
            ...
            <div className={cn("relative min-h-14 overflow-visible", compact && "min-h-10 min-w-0")}>
              <div
                ref={videosBar}
                className={cn("w-full overflow-x-auto overflow-y-visible overscroll-contain py-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden", compact && "py-0.5")}
              >
                <div className={cn("flex min-h-full min-w-full items-center pl-0.5 pr-1.5", compact ? "gap-1.5" : "gap-2")}>
                  {topFilteredLive.map((video, index) => (
                    <div key={`${video.id || video.link || "video"}-${index}`} className="group relative flex shrink-0 items-center" title={video.title} draggable onDragStart={(event) => dragVideo(event, video)}>
                      <Button type="button" variant="ghost" className="relative h-auto w-auto rounded-full p-0" onClick={() => handleVideoClick(video)}>
                        {video?.link && !compact ? <Badge variant="secondary" className="absolute left-0 top-0 z-10"><TwitchIcon className="size-3.5" /></Badge> : null}
                        {!compact ? <Badge variant="secondary" className="absolute bottom-[-2px] right-[-2px] z-10">{formatDurationLive(video)}</Badge> : null}
                        {video.channel?.id ? <ChannelImg channel={video.channel} size={compact ? 28 : 36} noLink className="bg-muted" /> : <div className={cn("flex items-center justify-center overflow-hidden rounded-full bg-muted text-muted-foreground", compact ? "size-7" : "size-9")}><icons.CircleUser className="size-4" /></div>}
                      </Button>
                    </div>
                  ))}
                </div>
