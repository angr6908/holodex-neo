...
import {
  type MultiviewVideoCellHandle,
  useRegisterMultiviewVideoCell,
} from "@/lib/multiview-video-cells";
...
export function VideoCell({ item, onDelete }: { item: any; onDelete?: (id: string) => void }) {
  ...
  const exposed = useMemo<MultiviewVideoCellHandle>(
    () => ({
      ...
      get isTwitchVideo() {
        return isTwRef.current;
      },
      get isFastFoward() {
        return rateRef.current !== 1;
      },
      get currentTime() {
  ...
  const ytClass =
    "absolute inset-0 h-full w-full overflow-hidden [&>div]:absolute [&>div]:inset-0 [&>div]:h-full [&>div]:w-full [&>div>iframe]:absolute [&>div>iframe]:inset-0 [&>div>iframe]:h-full [&>div>iframe]:w-full [&>iframe]:absolute [&>iframe]:inset-0 [&>iframe]:h-full [&>iframe]:w-full";
  const twClass =
    "absolute inset-0 h-full w-full overflow-hidden [&>iframe]:absolute [&>iframe]:inset-0 [&>iframe]:h-full [&>iframe]:w-full";

  return (
    <div
      key={`uid-${uid}`}
      className="flex h-full max-h-full min-h-0 w-full grow basis-full shrink flex-col overflow-hidden"
    >
