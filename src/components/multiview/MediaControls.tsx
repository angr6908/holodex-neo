...
export function MediaControls({ open }: { open?: boolean }) {
  ...
  const allVolume =
    cells.length && cells.every((cell) => cell.volume === cells[0].volume) ? cells[0].volume : 0;
  ...
  const renderAvatar = (cell: any) => {
    ...
    return channelId?.startsWith("UC") ? (
      <Link href={`/channel/${channelId}`}>{avatar}</Link>
    ) : (
      avatar
    );
  };
    ...
    <FieldGroup className="gap-3">
      <Field orientation="horizontal" className="items-center">
        ...
        <Slider
          ...
          step={1}
          value={[allVolume]}
          onValueChange={(value) => setAllVolume(Array.isArray(value) ? (value[0] ?? 0) : value)}
        />
      </Field>
              ...
              <Item key={cell.id} variant="outline" size="sm" className="items-start">
                ...
                <ItemContent className="min-w-0 gap-2">
                  <ItemTitle className="max-w-full truncate">
                    {cell.video?.title || cell.video?.channel?.name}
                  </ItemTitle>
                  <ItemActions>
                    <ButtonGroup>
                      <Button
                    ...
                    <Slider
                      ...
                      value={[cell.volume]}
                      onValueChange={(value) =>
                        cell.setVolume(Array.isArray(value) ? (value[0] ?? 0) : value)
                      }
                    />
                  </ItemActions>
                </ItemContent>
      ...
      <Field orientation="horizontal" className="rounded-lg border p-2.5">
        <Checkbox
          checked={store.muteOthers}
          onCheckedChange={(checked) => store.setMuteOthers(checked === true)}
        />
        <FieldContent>
