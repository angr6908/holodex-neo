...
import { ChevronUp, ClipboardCheck, ClipboardPlus, Menu, MoreVertical, Share2, type LucideIcon } from "lucide-react";
...
export function MultiviewToolbar({ compact = false, buttons = [], onCollapse, children, left, extraButtons }: { compact?: boolean; buttons?: readonly ToolbarButton[]; onCollapse?: () => void; children?: React.ReactNode; left?: React.ReactNode; extraButtons?: React.ReactNode }) {
      ...
      <div className="flex min-h-14 items-center gap-2 py-1">
        <div className="shrink-0 self-center">
          <Popover open={navMenuOpen} onOpenChange={setNavMenuOpen}>
            <PopoverTrigger
              render={<Button type="button" variant="ghost" size="icon" title={t("views.multiview.openNavigation")} />}
            >
        ...
        <div className="flex min-w-0 flex-wrap items-center justify-end gap-2 self-center">
          {extraButtons}
          {buttons.filter((button) => !button.collapse).map((button, index) => (
            <Button key={`mv-button-${index}`} type="button" variant="ghost" size="icon" title={button.tooltip} onClick={button.onClick}>
              <button.icon />
            </Button>
          ))}
          {!compact ? (
            <Popover open={shareDialog} onOpenChange={setShareDialog}>
              <PopoverTrigger
                render={<Button type="button" variant="ghost" size="icon" title={t("views.multiview.shareLayout")} />}
              >
                <Share2 />
              </PopoverTrigger>
              <PopoverContent align="end" sideOffset={8} className="w-[min(80vw,24rem)]">
                <div className="relative flex items-center">
                  ...
                  <div className="absolute right-1.5 top-1/2 -translate-y-1/2">
                    <Button type="button" variant="ghost" size="icon" onClick={() => startCopyToClipboard(exportURL)}>{doneCopy ? <ClipboardCheck /> : <ClipboardPlus />}</Button>
                  </div>
                </div>
              </PopoverContent>
            </Popover>
          ...
          {collapseButtons.length ? (
            <Popover open={collapsedMenuOpen} onOpenChange={setCollapsedMenuOpen}>
              <PopoverTrigger
                render={<Button type="button" variant="ghost" size="icon" title={t("component.common.moreActions")} />}
              >
                <MoreVertical />
              </PopoverTrigger>
              <PopoverContent align="end" sideOffset={8} className="w-auto min-w-[15rem] p-2">
                {collapseButtons.map((button, index) => (
                  <Button key={`mv-collapsed-${index}`} type="button" variant="ghost" onClick={() => handleCollapsedButton(button)}>
                    <button.icon />
          ...
          <Button type="button" variant="ghost" size="icon" title={t("views.multiview.collapseToolbar")} onClick={onCollapse}>
            <ChevronUp />
          </Button>
        </div>
      </div>
