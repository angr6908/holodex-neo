...
export function ChatCell({
  ...
  const videoCellId = useMemo(
    () =>
      Object.keys(store.layoutContent).find(
        (key) => store.layoutContent[key]?.video === currentVideo,
      ),
    [store.layoutContent, currentVideo],
  );
  const currentTime = videoCellId ? store.layoutContent[videoCellId]?.currentTime || 0 : 0;
  const channels = activeVideos.map((video: any, index: number) => ({
  ...
  function handleVideoUpdate(update: any) {
    ...
    if (v.status !== update.status || v.start_actual !== update.start_actual)
      store.setLayoutContentWithKey({ id: videoCellId, key: "video", value: { ...v, ...update } });
  }
    ...
    <div className="flex h-full max-h-full min-h-0 w-full grow basis-full shrink flex-col overflow-hidden">
        ...
        {currentVideo ? (
          currentVideo.type === "twitch" ? (
            ...
            <WatchLiveChat
              ...
              modelValue={{ showTlChat, showYtChat }}
              video={currentVideo}
              fluid
              scale={1}
      ...
      <div className="flex min-h-[26px] w-full shrink-0 items-center gap-1 px-1 py-0.5">
        <ToggleGroup
          ...
          size="sm"
          value={chatModes}
          onValueChange={setChatModes}
          className="min-w-0 flex-1"
        >
          <ToggleGroupItem value="yt">
            <MessageCircle />
            {cellWidth > 200 ? t("component.common.chat") : null}
          </ToggleGroupItem>
          <ToggleGroupItem value="tl">
            <Languages />
            {cellWidth > 200 ? "TL" : null}
          </ToggleGroupItem>
        </ToggleGroup>
        <Button
          type="button"
