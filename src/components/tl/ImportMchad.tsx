...
export function ImportMchad({ onClose }: { onClose?: (payload: { upload?: boolean }) => void }) {
  ...
  async function claimAll() {
    try {
      setWorking(true);
      const res = await api.claimMchadMigrate(app.userdata.jwt!, room, pass);
      if (res.status === 200) setClaimSuccess(true);
    } catch (error: any) {
      setClaimErrorMsg(error.response?.data?.message || error.message);
    }
    setWorking(false);
  }
  return (
    <div className="space-y-5 p-6">
      <h2 className="text-lg font-normal">{t("views.tlManager.importFromMchad")}</h2>
      {claimSuccess ? (
        <Alert>
      ...
      <FieldGroup>
        ...
        <Button variant="secondary" className="w-full" onClick={checkAvailable}>
          {t("views.tlManager.importMchad.login")}
        </Button>
            ...
            {archiveData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={3} className="py-8 text-center text-muted-foreground">
                  {t("views.tlManager.importMchad.noArchives")}
                </TableCell>
              </TableRow>
            ) : null}
          </TableBody>
