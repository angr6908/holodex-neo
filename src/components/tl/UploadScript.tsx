...
export function UploadScript({
  ...
  function fileChange(f: File | undefined) {
    ...
    const handle = (process: (data: string) => void) => {
      reader.onload = (res) => process((res.target as FileReader).result as string);
      reader.readAsText(f);
    };
  ...
  async function sendData() {
    ...
    const processes: any[] = existing.data.map((e: any) => ({
      type: "Delete",
      data: { id: e.id },
    }));
    entries.forEach((e, i) => {
    ...
    api
      .postTLLog({ videoId: videoData.id, jwt: user.jwt!, body: processes, lang: TLLang.value })
      .then(({ status }: any) => {
        if (status === 200) onClose?.({ upload: true });
      })
  ...
  return (
    <div className="space-y-5 p-6">
      ...
      <Field className="gap-2">
        ...
        <Select
          value={TLLang.value}
          onValueChange={(v) => setTLLang(TL_LANGS.find((i) => i.value === v) || TL_LANGS[0])}
        >
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="destructive" className="ml-auto" disabled={!parsed} onClick={sendData}>
