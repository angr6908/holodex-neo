...
export function ExportToFile({ videoData }: { videoData: any }) {
      ...
      .then(({ status, data }: any) => {
        if (status === 200) {
          const fetchChat = videoData.start_actual
            ? data
                .filter((e: any) => e.timestamp >= videoData.start_actual)
                .map((e: any) => {
  ...
  return (
    <div className="space-y-5 p-6">
      <h2 className="text-center text-lg font-normal">{t("views.tlManager.download")}</h2>
      <div className="space-y-2">
        ...
        >
          <SelectTrigger id="tl-export-lang" className="w-full">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {TL_LANGS.map((item) => (
      ...
      <div className="flex flex-wrap justify-center gap-3">
        ...
        <Button
          type="button"
          onClick={() => downloadFile("ass", buildTlAssExport(entries, profile))}
        >
          .ass
        </Button>
        <Button
          type="button"
          onClick={() => downloadFile("ttml", buildTlTtmlExport(entries, profile))}
        >
          .ttml
        </Button>
      </div>
