...
export function TlEntryRow({
  ...
  const end = useRealTime
    ? dayjs(realTime + duration).format("h:mm:ss.SSS A")
    : formatTlTimestamp(time + duration);
...
function TlExportButtons({ onExportSrt, onExportAss, onExportTtml }: TlExportButtonsProps) {
  return (
    <div className="flex flex-wrap justify-center gap-3">
      ...
      <Button type="button" onClick={onExportAss}>
        .ass
      </Button>
      <Button type="button" onClick={onExportTtml}>
        .ttml
      </Button>
    </div>
  );
...
export function ScriptEditorExportToFile({
  entries = [],
  profile = [],
  title = "Holodex",
}: {
  ...
  return (
    <div className="space-y-4 p-4">
      <h2 className="text-center text-xl font-normal text-foreground">
      ...
      <TlExportButtons
        onExportSrt={() => downloadTextFile(`${title}.srt`, buildTlSrtExport(entries))}
        onExportAss={() => downloadTextFile(`${title}.ass`, buildTlAssExport(entries, profile))}
        onExportTtml={() => downloadTextFile(`${title}.ttml`, buildTlTtmlExport(entries, profile))}
      />
    </div>
  );
}
