...
import { ArrowDown, Ban, Clapperboard, Heart, List, LayoutGrid, Radio, UserMinus } from "@/lib/icons";
...
export function ChannelsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => { document.title = `${t("component.mainNav.channels")} - Holodex`; }, [t]);

  const sortOptions = useMemo<SortOpt[]>(() => [
    { text: t("views.channels.sortOptions.subscribers"), value: "subscribers", query_value: { sort: "subscriber_count", order: "desc" } },
    ...((category === Tabs.VTUBER || category === Tabs.FAVORITES) && singleOrg
      ? [{ text: t("views.channels.sortOptions.group"), value: "group", query_value: { sort: "suborg", order: "asc" } }] : []),
    ...((multiOrg || allVtubers)
      ? [{ text: t("views.channels.sortOptions.org"), value: "org", query_value: { sort: "suborg", order: "asc", groupByOrg: true } }] : []),
    { text: t("views.channels.sortOptions.videoCount"), value: "video_count", query_value: { sort: "video_count", order: "desc" } },
    ...((category === Tabs.VTUBER || category === Tabs.FAVORITES)
      ? [{ text: t("views.channels.sortOptions.clipCount"), value: "clip_count", query_value: { sort: "clip_count", order: "desc" } }] : []),
    ...((category === Tabs.VTUBER || category === Tabs.SUBBER)
      ? [{ text: t("views.channels.sortOptions.recentUpload"), value: "recently_added", query_value: { sort: "created_at", order: "desc" } }] : []),
  ], [t, category, singleOrg, multiOrg, allVtubers]);

  const currentSort = sortOptions.find((o) => o.value === sortValue) || sortOptions.find((o) => o.value === DEFAULT_SORT) || sortOptions[0];
  ...
  const setSort = useCallback((v: string) => setState((p) => ({
    ...p, sort: { ...p.sort, [p.category]: sortOptions.find((o) => o.value === v) ? v : DEFAULT_SORT },
  })), [sortOptions]);
  ...
  const loadFn = useCallback(async (offset: number, limit: number) => {
    ...
    const base: Record<string, any> = { limit, offset, type, ...(type === CHANNEL_TYPES.SUBBER && { lang: langsKey }), ...apiSv };
    if (type === CHANNEL_TYPES.VTUBER) {
      if (selectedOrgs.length > 1) {
        const res = await Promise.all(selectedOrgs.map((org) => api.channels({ ...base, org }).then((r: any) => r.data)));
  ...
  return (
    <section className={cn("flex min-h-[70vh] flex-col", !embedded && "mx-auto min-h-screen w-full max-w-[1600px] px-3 pb-10 pt-[var(--nav-total-height,120px)] sm:px-5")}
      onTouchStart={swipeTabs.onTouchStart} onTouchEnd={swipeTabs.onTouchEnd}
