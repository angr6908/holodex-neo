...
export function ChannelList({
  ...
  const isHidden = (g: string) =>
    app.settings.hiddenGroups?.[app.currentOrg.name]?.includes(g.toLowerCase()) ?? false;
    ...
    !channel ? null : (
      <Item className="flex items-center gap-3 rounded-none px-4 py-2.5 transition-colors hover:bg-muted/30">
        <div className="shrink-0">
          <ChannelImg channel={channel} size={48} />
        </div>
        <ChannelInfo channel={channel} includeVideoCount={includeVideoCount}>
          {isXs ? (
            <ChannelSocials channel={channel} className="mt-1 justify-start p-0" showDelete />
          ) : null}
        </ChannelInfo>
    ...
    <SectionPanel
      ...
      actions={
        <>
          ...
          >
            <icons.Heart
              className={cn("size-4", g.allFavorited && app.isLoggedIn && "text-primary")}
            />
