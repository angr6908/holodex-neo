  ...
  return (
    <Card className="group flex h-full w-full flex-col gap-0 overflow-hidden rounded-xl border border-border/60 bg-card/50 p-0 shadow-none ring-0">
      <Link
        ...
        className="flex flex-col items-center gap-1.5 px-3 pb-2.5 pt-4 no-underline"
      >
        ...
        <div className="w-full min-w-0 text-center">
          <div className="truncate text-[15px] font-medium leading-tight text-foreground">
            {channel.inactive ? (
              <icons.GraduationCap className="mr-0.5 inline-block size-4 align-[-2px]" />
            ) : null}
            {channelName}
          </div>
          {channel.org ? (
        ...
        {channel.video_count ? (
          <span className="whitespace-nowrap">
            {t("component.channelInfo.videoCount", {
              arg0: formatCount(channel.video_count, app.settings.lang),
            })}
          </span>
        ) : null}
