...
import themeSet, { applyThemeColor, readStoredThemeId } from "@/lib/themes";
import { cn } from "@/lib/utils";

const sectionLabelClass =
  "text-[0.68rem] font-normal uppercase leading-normal tracking-[0.16em] text-muted-foreground";
...
export function SettingsPage({ className = "" }: { className?: string }) {
  ...
  const selectedTheme =
    themeOptions.find((theme) => theme.id === Number(themeId)) || themeOptions[0];
  const themeName = (name?: string) =>
    name === "Default"
      ? t("views.settings.defaultTheme")
      : name || t("views.settings.defaultTheme");
  ...
  function checkChip(
    ...
    return (
      <Toggle
        ...
        disabled={disabled}
        variant="outline"
        className={cn("min-w-fit flex-1 justify-center", disabledClass && "opacity-60")}
        aria-label={label}
  ...
  return (
    <div
      className={cn(
        "min-h-0 flex-1 space-y-6 overflow-y-auto overflow-x-hidden p-4 sm:p-5",
        className,
      )}
    >
      <div className="space-y-3">
        <div className="flex flex-wrap items-end gap-2">
              ...
              <SelectContent>
                {langs.map((lang) => (
                  <SelectItem key={lang.val} value={lang.val}>
                    {(lang as any).credit
                      ? `${(lang as any).display} - ${(lang as any).credit}`
                      : (lang as any).display}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className={settingControlClass}>
            ...
            <ToggleGroup
              ...
              onValueChange={(value) =>
                value[0] && app.patchSettings({ defaultOpen: value[0] as any })
              }
            >
        ...
        {overrideLanguage ? (
          <Alert onClick={clearOverrideLanguage}>
            <AlertDescription>
              {t("views.settings.languageOverrideActive", {
                language:
                  langs.find((x) => x.val === overrideLanguage)?.display || overrideLanguage,
              })}
            </AlertDescription>
          </Alert>
        ) : null}
        ...
        <div className="flex flex-col gap-[0.45rem]">
          ...
          <div className="flex flex-wrap items-stretch gap-2">
            {checkChip(t("views.settings.openOnHolodexLabel"), !app.settings.redirectMode, (v) =>
              app.patchSettings({ redirectMode: !v }),
            )}
            {checkChip(
              t("views.settings.darkModeLabel"),
              app.settings.darkMode,
              (v) => app.patchSettings({ darkMode: v }),
            ...
            {checkChip(
              ...
              app.settings.followSystemTheme,
              (v) => app.patchSettings({ followSystemTheme: v }),
            )}
            {checkChip(t("views.settings.useEnglishNameLabel"), app.settings.useEnglishName, (v) =>
              app.patchSettings({ useEnglishName: v }),
            )}
          </div>
        </div>
      </div>
