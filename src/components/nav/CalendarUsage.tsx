...
import { ClipboardPlus, Filter, MessageSquare, SearchCheck, Users } from "@/lib/icons";
...
export function CalendarUsage({
  ...
  const itemsByValue = useMemo(
    () => new Map([...query, ...fromApi].map((item) => [item.value, item])),
    [fromApi, query],
  );

  const preferEnglishName = app.settings.useEnglishName;
  const liveCalendarURL = useMemo(() => {
    const bucket = query.reduce((b: any, { type, value }: any) => {
      b[type] ||= [];
      b[type].push(value);
      return b;
    }, {});
  ...
  async function runSearch(formatted: string) {
    ...
    try {
      ...
      const next = (res.data || [])
        ...
        .filter((x: any) => x.type === "topic" || x.type === "channel" || x.type === "org");
      setFromApi(next);
    } catch (e) {
      if (current === requestId.current) setFromApi([]);
      console.error(e);
    } finally {
  ...
  useEffect(() => {
    if (Array.isArray(initialQuery)) setQuery(initialQuery);
  }, [initialQuery]);
  useEffect(() => {
    const formatted = (searchInput || "").trim().replace("#", "");
    if (!formatted || encodeURIComponent(formatted).length <= 1) {
      setFromApi([]);
      setIsLoading(false);
      return;
    }
  ...
  function constrainQuery(next: any[]) {
    if (next.filter((x) => x.type === "org").length > 1)
      next.splice(
        next.findIndex((x) => x.type === "org"),
        1,
      );
    return next;
  }
    ...
    <div className="space-y-4">
      <div className="relative">
        <div className="flex flex-col gap-[0.45rem]">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t("component.search.filterByTopicOrgChannel")}</span>
          </div>
          <Combobox
            multiple
            items={resultValues}
            value={query.map((item) => item.value)}
            ...
            <ComboboxChips ref={filterAnchor}>
              {query.map((item) => (
                <ComboboxChip key={item.value} className="max-w-full">
                  <span className="shrink-0 text-muted-foreground">{i18nItem(item.type)}</span>
                  <span className="min-w-0 truncate whitespace-nowrap">{item.text}</span>
                </ComboboxChip>
              ))}
                  ...
                  return (
                    <ComboboxItem key={value} value={value} index={index}>
                      {itemIcon
                        ? (() => {
                            const C = itemIcon;
                            return <C className="size-4 text-muted-foreground" />;
                          })()
                        : null}
      ...
      <div className="space-y-2">
        <Label className="text-sm text-muted-foreground">
          {t("component.calendar.liveCalendar")}
        </Label>
        <Button
          type="button"
          variant="outline"
          className="h-auto w-full justify-between text-left font-normal whitespace-normal"
          onClick={(e) => {
            e.stopPropagation();
            copyToClipboard(liveCalendarURL);
          }}
        >
      ...
      {app.isLoggedIn && showFavoritesCalendar ? (
        <div className="space-y-2">
          <Label className="text-sm text-muted-foreground">
            {t("component.calendar.favoritesCalendar")}
          </Label>
          <Button
            type="button"
            variant="outline"
            ...
            onClick={(e) => {
              e.stopPropagation();
              copyToClipboard(favoritesCalendarURL);
            }}
          >
