...
export function NavUserMenu() {
  ...
  return <Popover open={menuOpen} onOpenChange={setMenuOpen}>
    ...
    >
      {user ? <Avatar className="block size-full"><AvatarImage src={avatarUrl} alt={t("component.userMenu.userAvatar")} /><AvatarFallback>{userTag.charAt(0).toUpperCase()}</AvatarFallback></Avatar> : <LogIn className="h-5 w-5" aria-hidden="true" />}
    </PopoverTrigger>
    <PopoverContent align="end" sideOffset={8} className={user ? "max-h-[80dvh] w-[min(92vw,20rem)] overflow-y-auto p-0" : "w-[min(92vw,18rem)] p-0"}>
      {!user ? <div className="p-5 text-center"><h2 className="mb-5 text-base font-semibold tracking-tight text-foreground">{t("component.mainNav.login")}</h2><div className="space-y-3" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-3 px-3 py-3"><Avatar className="h-10 w-10 shrink-0"><AvatarImage src={avatarUrl} alt={t("component.userMenu.userAvatar")} /><AvatarFallback>{userTag.charAt(0).toUpperCase()}</AvatarFallback></Avatar><div className="min-w-0 flex-1">{editingUsername ? <div className="flex items-center gap-1.5"><Input value={usernameInput} onChange={(e) => setUsernameInput(e.target.value)} className="flex-1 text-xs" onKeyDown={(e) => { if (e.key === "Enter") saveUsername(); }} onClick={(e) => e.stopPropagation()} /><Button size="icon-xs" onClick={saveUsername}><Check className="size-3.5" /></Button><Button variant="secondary" size="icon-xs" onClick={cancelUsernameEdit}><XIcon className="size-3.5" /></Button></div> : <><div className="flex items-center gap-1"><div className="truncate text-sm font-semibold text-foreground">{userTag}</div><Button variant="ghost" size="icon-xs" onClick={(e) => { e.stopPropagation(); startUsernameEdit(); }}><Pencil className="size-3.5" /></Button></div><div className="text-xs text-muted-foreground">{userPts}</div></>}</div><Button variant="ghost" size="icon-xs" className="ml-auto shrink-0 self-start" title={t("component.mainNav.logout")} onClick={handleLogout}><LogOut className="size-4" /></Button></div>
