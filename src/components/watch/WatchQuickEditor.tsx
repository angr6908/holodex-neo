...
export function WatchQuickEditor({ video }: { video: Record<string, any> }) {
  ...
  const channelLabels = useMemo(
    () => new Map(searchResults.map((item) => [item.id, channelDisplayName(item, useEnglishName)])),
    [searchResults, useEnglishName],
  );
  ...
  function applyDeleteMentions() {
    ...
    api
      ...
      .catch((e: any) =>
        showError(e.response?.data.message || e.message || t("component.form.error")),
      )
      ...
      .then(({ data }: any) => {
        ...
        showSuccess(
          t("views.editor.channelMentions.addSuccess", {
            channel: channelDisplayName(channel, useEnglishName),
          }),
        );
        updateMentions();
      })
                  ...
                  {() => (
                    <div className="absolute inset-0 flex items-center justify-center rounded-full">
                      <Button
                        type="button"
                        size="icon"
                        variant={isAddedToDeletionSet(item.id) ? "destructive" : "ghost"}
                        className="h-8 w-8"
                        onClick={(event) => {
                          event.stopPropagation();
                          toggleDeletion(item.id);
                        }}
                      >
                        {isAddedToDeletionSet(item.id) ? (
        ...
        {video.type === "stream" || video.type === "placeholder" ? (
          <Card className="min-w-[260px] max-w-sm flex-1 p-4">
            ...
            <div className="mb-3 text-sm">
              {currentTopic || t("views.editor.changeTopic.unset")}
            </div>
            <Combobox
              items={topicValues}
              value={newTopic}
            ...
            <Button type="button" size="sm" className="mt-3" onClick={saveTopic}>
