...
export function VideoCardMenu({ video, close }: { video: any; close: () => void }) {
  ...
  const isLive = !!(video.status !== "past" && (video.status === "live" || Date.parse(video.start_scheduled) < Date.now()));
  ...
  const copyLink = () => { navigator.clipboard?.writeText(`${window.origin}/watch/${video.id}`); setDoneCopy(true); };

  const openTl = () => {
    if (!app.userdata?.user) return openUserMenu();
    if (isLive || video.status === "upcoming") router.push(`/tlclient?video=${encodeURIComponent(video.type === "placeholder" ? video.link : `YT_${video.id}`)}`);
      ...
      {video.type !== "placeholder" ? (
        <>
          <Button nativeButton={false}
            render={<a target="_blank" href={`https://youtu.be/${video.id}`} onClick={(e) => { e.stopPropagation(); close(); }} />}
            variant="ghost" className={ITEM_CLASS}>
            <icons.YoutubeIcon className="h-4 w-4" />{t("views.settings.redirectModeLabel")}
          </Button>
          {video.status === "upcoming" ? gCalBtn : null}
          <Button nativeButton={false}
            render={<Link href={`/edit/video/${video.id}${video.type !== "stream" ? "/mentions" : "/"}`} onClick={close} />}
            variant="ghost" className={ITEM_CLASS}>
            <icons.Pencil className="h-4 w-4" />{t("component.videoCard.edit")}
          </Button>
          {video.type !== "clip" ? (
            <Button nativeButton={false}
              render={<Link href={`/multiview/AAUY${video.id}%2CUAEYchat`} onClick={close} />}
              variant="ghost" className={ITEM_CLASS}>
              <icons.LayoutDashboard className="h-4 w-4" />{t("component.mainNav.multiview")}
            </Button>
          ) : null}
          <Collapsible open={showPlaylist} onOpenChange={setShowPlaylist}>
            <CollapsibleTrigger render={<Button type="button" variant="ghost" className={ITEM_CLASS} />}>
              <icons.ListPlus className="h-4 w-4" />{t("component.mainNav.playlist")}
          ...
          <Button type="button" variant={doneCopy ? "default" : "ghost"} className={ITEM_CLASS}
            onClick={(e) => { e.stopPropagation(); copyLink(); close(); }}>
            <icons.ClipboardPlus className="h-4 w-4" />{t("component.videoCard.copyLink")}
          </Button>
        </>
      ) : (video.status === "upcoming" ? gCalBtn : null)}
      <Button type="button" variant="ghost" className={ITEM_CLASS} onClick={() => { openTl(); close(); }}>
        <icons.Pencil className="h-4 w-4" />
        {isLive || video.status === "upcoming" ? t("component.videoCard.openClient") : t("component.videoCard.openScriptEditor")}
      </Button>
      {isPast ? (
        <Button type="button" variant="ghost" className={ITEM_CLASS} onClick={() => { openUpload(); close(); }}>
          <icons.ClipboardCopy className="h-4 w-4" />{t("component.videoCard.uploadScript")}
        </Button>
      ) : null}
      {isChattable ? (
        <Button type="button" variant="ghost" className={ITEM_CLASS} onClick={() => { openChatPopout(); close(); }}>
          <icons.ExternalLink className="h-4 w-4" />{t("component.videoCard.popoutChat")}
        </Button>
      ) : null}
      <Button type="button" variant="ghost" className={ITEM_CLASS} onClick={() => { app.setReportVideo(video); close(); }}>
        <icons.Flag className="h-4 w-4" />{t("component.reportDialog.title")}
      </Button>
      {app.isSuperuser ? <div className="pt-1"><WatchQuickEditor video={video} /></div> : null}
    </div>
  );
}
