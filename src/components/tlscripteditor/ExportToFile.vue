<template>
  <div class="space-y-4 p-4">
    ...
    <h4 class="text-center text-sm text-[color:var(--color-muted-foreground)]">
      {{ entries.length + " entries, " + profile.length + " profile." }}
    </h4>
    <div class="flex flex-wrap justify-center gap-3">
      <UiButton type="button" @click="exportSrt()">
        .srt
      </UiButton>
      <UiButton type="button" @click="exportAss()">
        .ass
      </UiButton>
      <UiButton type="button" @click="exportTTML()">
        .ttml
      </UiButton>
    </div>
  </div>
...
function exportAss() {
  const entries = props.entries as any[];
  ...
  for (let i = 0; i < profile.length; i += 1) {
    ...
    if (profile[i].useCC) {
      writeStream += profile[i].CC.substring(5, 7) + profile[i].CC.substring(3, 5) + profile[i].CC.substring(1, 3);
    } else {
      writeStream += "FFFFFF";
    }
    ...
    if (profile[i].useOC) {
      writeStream += profile[i].OC.substring(5, 7) + profile[i].OC.substring(3, 5) + profile[i].OC.substring(1, 3);
    } else {
      writeStream += "000000";
    }
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    if (i === entries.length - 1) {
      writeStream += `${stringifyTime(entries[i].Time + 3000, false)},`;
    } else {
      writeStream += `${stringifyTime(entries[i].Time + entries[0].Duration, false)},`;
    }
...
function exportTTML() {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    writeStream += `\t\t<p t="${
      (entries[i].Time + 1).toString()
    }" d="`;

    if (i === entries.length - 1) {
      writeStream += `${(entries[i].Time + 3001).toString()}"`;
    } else {
      writeStream += `${(entries[i].Time + 1 + entries[i].Duration).toString()}"`;
    }

    writeStream += " wp=\"1\" ws=\"1\"><s p=\"1\"></s><s p=\"";

    writeStream += `${((entries[i].Profile * 2) + 2).toString()}"> ${entries[i].SText} </s><s p="1"></s></p>\n`;
    ...
    writeStream += `${((entries[i].Profile * 2) + 3).toString()}"> ${entries[i].SText} </s><s p="1"></s></p>\n`;
  }
  ...
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = `${props.title}.ttml`;
  link.click();
  link.remove();
}
...
function exportSrt() {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    if (i === entries.length - 1) {
      writeStream += `${stringifyTime(entries[i].Time + 3000, true)}\n`;
    } else {
      writeStream += `${stringifyTime(entries[i].Time + entries[i].Duration, true)}\n`;
    }
