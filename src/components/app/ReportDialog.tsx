...
export function ReportDialog() {
  ...
  const reasonList = useMemo(() => {
    ...
    return [
      ...
      {
        text: t("component.reportDialog.reasons.6", { arg0: vt, arg1: app.currentOrg.name }),
  ...
  const filteredReasons = reasonList.filter((r) => {
    ...
    if (
      r.orgReq &&
      (!app.currentOrg ||
        app.currentOrg.name === ALL_VTUBERS_ORG ||
        app.currentOrg.name === video.channel?.org ||
        !isHome)
    )
      return false;
  ...
  const removeFromDelSet = (id: string) =>
    setDelSet((p) => {
      const n = new Set(p);
      n.delete(id);
      return n;
          ...
          {video ? (
            <div className="space-y-4">
              ...
              {isCollab ? (
                <Alert>
                  <AlertDescription>
                    {t("component.reportDialog.collabing", { org: app.currentOrg.name })}
                  </AlertDescription>
                </Alert>
              ) : null}
                ...
                <div className="space-y-2">
                  <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                    ...
                    <span className="ml-2 text-primary">
                      {video?.topic_id || t("component.form.none")}
                    </span>
                  </div>
                  <Select
                    value={sugTopic || "__unset__"}
                    onOpenChange={(o) => {
                      if (o) void loadTopics();
                    }}
                    onValueChange={(v) => setSugTopic(v === "__unset__" ? false : v)}
                  >
              ...
              {reasons.includes("This video does not belong to the org") &&
              !collabsHidden &&
              video?.type !== "clip" ? (
                <div className="space-y-2">
