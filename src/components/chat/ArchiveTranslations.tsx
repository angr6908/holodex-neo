...
export function ArchiveTranslations({
  ...
  const itemClass = (index: number) =>
    index === curIndex
      ? "relative z-0 [&_.tl-message]:relative [&_.tl-message]:z-0 [&_.tl-message]:before:absolute [&_.tl-message]:before:left-0 [&_.tl-message]:before:top-[-1px] [&_.tl-message]:before:z-[-1] [&_.tl-message]:before:h-full [&_.tl-message]:before:w-full [&_.tl-message]:before:bg-primary/70 [&_.tl-message]:before:bg-cover [&_.tl-message]:before:opacity-25 [&_.tl-message]:before:content-['']"
      : app.settings.liveTlHideSpoiler && index > curIndex
        ? "hidden"
        : "";
  ...
  const body = (
    <div
      ...
      onClick={handleClick}
    >
      {dividedTLs.map((item, index) => (
        <div
          key={item.key || `${item.timestamp}${item.message}${item.name}`}
          data-archive-index={index}
          className={itemClass(index)}
        >
          <ChatMessage source={item} />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <Card
      className={cn(
        "box-border flex min-h-0 w-full flex-col overflow-hidden p-0 text-sm",
        className,
      )}
    >
      <div className="flex items-center justify-between gap-3 border-b px-3 py-2">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            title="-2s"
            onClick={() => setTimeOffsetSeconds((v) => v - 2)}
          >
          ...
          >{`${timeOffsetSeconds >= 0 ? "+" : ""}${timeOffsetSeconds}s`}</Button>
          <Dialog open={dialog} onOpenChange={setDialog}>
            <DialogContent className="max-w-xs">
              <Card>
                <div className="text-sm font-normal">
                  {t("views.scriptEditor.timeShift.timeOffset")}
                </div>
                <div className="flex flex-col gap-2 text-sm">
                  <Label htmlFor="archive-tl-time-offset">
                    {t("views.scriptEditor.timeShift.offset")}
                  </Label>
                  <div className="flex items-center gap-2">
                    <Input
                      id="archive-tl-time-offset"
                      value={timeOffsetSeconds}
                      type="number"
                      className="flex-1"
                      onChange={(event) => setTimeOffsetSeconds(Number(event.target.value))}
                    />
                    <span className="text-muted-foreground">
                      {t("views.scriptEditor.timeShift.secondsShort")}
                    </span>
                  </div>
                </div>
              </Card>
          ...
          >
            <Captions className={cn("size-4", showSubtitle ? "text-primary" : "")} />
          </Button>
          <Button
            variant="ghost"
            size="icon"
      ...
      {expanded ? null : body}
      {showSubtitle && subtitleTarget
        ? createPortal(<WatchSubtitleOverlay messages={toDisplay} />, subtitleTarget)
        : null}
    </Card>
  );
}
